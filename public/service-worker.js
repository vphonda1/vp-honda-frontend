// ════════════════════════════════════════════════════════════════════════════
// VP Honda Service Worker v2.5.0
// Fixed clone error + Complete Push Notifications for Chat + Reminders intact
// ════════════════════════════════════════════════════════════════════════════
...
// ── PUSH NOTIFICATION (WhatsApp-like for chat) ─────────────────────────────
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title, {
      ...
      tag: data.tag || 'vp-chat',
      data: { url: data.url || '/team-chat' },
      vibrate: [200, 100, 200],
      requireInteraction: true   // WhatsApp-like persistent
    })
  );
});

// ── NOTIFICATION CLICK (both reminders & chat) ──────────────────────────────
...
function processSchedule(items = []) {
  ...
  console.log(`[SW] ${immediate.length} immediate + ${future.length} scheduled reminders`);
}

// ════════════════════════════════════════════════════════════════════════════
// BACKGROUND CHECK (from IndexedDB for reminders)
// ════════════════════════════════════════════════════════════════════════════
