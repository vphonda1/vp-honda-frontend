...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.startsWith('/api/') || url.hostname.includes('onrender.com')) {
    e.respondWith(fetch(request).then(res => {
      ...
      return res;
    }).catch(() => caches.match(request).then(c => c || new Response(JSON.stringify({ error:'Offline', message:'इंटरनेट नहीं है' }), { status:503, headers:{'Content-Type':'application/json'} }))));
    return;
  }
...
self.addEventListener('push', event => {
  let data = { title: 'नया मैसेज', body: 'कोई आपको मैसेज किया', url: '/team-chat' };
  if (event.data) {
    try { data = event.data.json(); } catch(e) { data.body = event.data.text(); }
  }
  event.waitUntil(self.registration.showNotification(data.title, {
    body: data.body, icon: '/icons/icon-192x192.png', badge: '/icons/icon-96x96.png',
    tag: data.tag || 'vp-chat', data: { url: data.url || '/team-chat' }, vibrate: [200,100,200], requireInteraction: true
  }));
});

self.addEventListener('notificationclick', event => {
  event.notification.close();
  const targetUrl = event.notification.data?.url || '/team-chat'; // यदि reminder होगा तो अलग से हैंडल होगा
...
self.addEventListener('periodicsync', e => { if (e.tag === 'vp-reminder-check') e.waitUntil(bgCheck()); });
self.addEventListener('sync', e => { if (e.tag === 'vp-reminder-sync') e.waitUntil(bgCheck()); });
...
function showNotif(title, body, data = {}) {
  ...
  return self.registration.showNotification(title, {
    ...
    vibrate: [200,100,200,100,300], tag: data.tag || `vp-${meta.tag}`,
    renotify: true, requireInteraction: data.requireInteraction || false,
    data: { url: data.url || '/reminders', type: data.type, ...data },
    actions: [{ action:'view', title:'👁️ देखें' }, { action:'whatsapp', title:'📱 WA भेजें' }, { action:'dismiss', title:'✕ बाद में' }]
  });
}
...
function processSchedule(items = []) {
  ...
  immediate.forEach((item, idx) => setTimeout(() => showNotif(item.title, item.body, { url: item.url || '/reminders', tag: item.tag || `vp-${idx}`, type: item.type, requireInteraction: item.requireInteraction }), idx * 1500));
  future.forEach(item => {
    const fireAt = new Date(item.fireAt).getTime();
    const delay = fireAt - now;
    if (delay <= 48 * 3600 * 1000) {
      const timer = setTimeout(() => showNotif(item.title, item.body, { url: item.url || '/reminders', tag: item.tag, type: item.type, requireInteraction: item.requireInteraction }), Math.min(delay, 2147483647));
...
async function bgCheck() {
  ...
  } catch(err) { console.log('[SW BG] Check failed:', err); }
}
