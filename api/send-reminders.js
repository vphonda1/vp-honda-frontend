    ...
    if (!typeFilter || typeFilter === 'service') {
      ...
      if (data.purchaseDate && !data.firstServiceDate) {
        ...
        if (!isParked(svcRid)) {
          ...
          if (dr >= -30) all.push({ id, rid: svcRid, type: 'service', customerName: nm, phone: ph, vehicleModel: vh, regNo, title: '🔧 1st Service Due', daysRemaining: dr });
        }
      } else {
        for (const svc of SERVICE_MAP) {
          ...
          if (doneDate && !data[nextKey]) {
            ...
            if (!isParked(svcRid)) {
              ...
              const dr   = Math.floor((due - todayMs) / DAY);
              if (dr >= -30) all.push({ id, rid: svcRid, type: 'service', customerName: nm, phone: ph, vehicleModel: vh, regNo, title: `🔧 ${svc.label} Due`, daysRemaining: dr });
            }
            break;
          }
        }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const newUrgent = urgent.filter(r => !alreadySentToday.includes(r.rid));
    ...
    const newSentIds = top.map(({ head }) => head.rid);
    try {
      await db.collection('notificationlogs').updateOne(
        { logKey },
