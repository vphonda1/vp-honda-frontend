...
const LEAD_COLLECTIONS = ['visitors', 'visitor', 'leads'];
...
function buildReminders(serviceData, typeFilter) {
  ...
  for (const data of serviceData) {
    ...
    // RTO और Insurance-renewal दोनों यहीं बनते हैं, पर type अलग-अलग है
    if (!typeFilter || typeFilter === 'insurance' || typeFilter === 'rto') {
      // ⚠️ RTO का अपना अलग type. पहले यह 'insurance' में मिला हुआ था, इसलिए
      // 5 बजे वाले RTO slot के लिए इसे अलग से छाँटा नहीं जा सकता था.
      ...
      if (data.insuranceDate && !data.rtoDoneDate && !isParked(rtoRid)) {
        ...
        if (dr >= 0 && dr <= 7) all.push({ id, rid: rtoRid, type: 'rto', customerName: nm, phone: ph, vehicleModel: vh, regNo, title: '🚗 RTO Pending', daysRemaining: dr });
      }
...
function shouldNotify(r, typeFilter) {
  if (typeFilter) {
    // 'insurance' चुनने पर renewal भी आए; 'rto' सिर्फ़ RTO
    const ok = r.type === typeFilter ||
               (typeFilter === 'insurance' && r.type === 'insurance-renewal');
    if (!ok) return false;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⭐ Showroom leads की "अगली call" भी notification में जाए.
    // बिकी / छूटी leads छोड़ दी जाती हैं.
    if (!typeFilter || typeFilter === 'followup') {
      const OPEN = ['new', 'contacted', 'quoted', 'negotiating'];
      const { data: leads } = await getCol(db, LEAD_COLLECTIONS);
      const t0 = new Date(); t0.setHours(0, 0, 0, 0);
      for (const L of leads) {
        if (!L?.nextFollowUp) continue;
        const st = L.stage || (L.converted ? 'won' : 'new');
        if (!OPEN.includes(st)) continue;
        const due = new Date(L.nextFollowUp); due.setHours(0, 0, 0, 0);
        if (isNaN(due)) continue;
        const dr = Math.floor((due - t0.getTime()) / 86400000);
        if (dr > 30 || dr < -60) continue;
        const lid = String(L.localId || L._id);
        reminders.push({
          id: lid.slice(-8), rid: `lead-${lid}`, type: 'followup',
          customerName: L.name || 'Lead', phone: L.phone || '',
          vehicleModel: L.interestedModel || '', regNo: '',
          title: '👤 Lead — call करनी है', daysRemaining: dr,
        });
      }
    }
    ...
    if (newUrgent.length === 0) {
      ...
      // ⚠️ FIX: पहले हर type के लिए अलग "सब clear" notification चली जाती थी —
      // दिन में 7 बार. आपको लगता कि notification का system चल तो रहा है, पर
      // असली reminder एक भी नहीं आया.
      //
      // "कुछ नहीं है" बताने के लिए notification भेजने का कोई फ़ायदा नहीं.
      // इसलिए अब all-clear भेजी ही नहीं जाती — बस चुपचाप लौट जाते हैं.
      return res.status(200).json({
        ok: true, mode: 'nothing-due', typeFilter: typeFilter || 'all',
        serviceDataCount: serviceData.length,
        नोट: 'इस type का कोई reminder due नहीं — कोई notification नहीं भेजी',
      });
    }
