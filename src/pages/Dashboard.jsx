...
export default function Dashboard({ user }) {
  ...
  const fetchAll = async (silent = false) => {
    ...
    try {
      const endpoints = ['customers', 'vehicles', 'invoices', 'jobcards', 'parts', 'salaries', 'reminders'];
      const results = await Promise.all(endpoints.map(e =>
        fetch(api(`/api/${e}`)).then(r => r.ok ? r.json() : []).catch(() => [])
      ));
      setCustomers(results[0] || []); setVehicles(results[1] || []); setInvoices(results[2] || []);
      setJobs(results[3] || []); setParts(results[4] || []); setSalaries(results[5] || []); setReminders(results[6] || []);
    } catch {}
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      inStock: vehicles.filter(v => v.status === 'in-stock' || !v.sold).length,
