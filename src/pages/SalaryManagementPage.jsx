...
export default function SalaryManagementPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [entRes, payRes, attRes, settRes] = await Promise.all([
        ...
        fetch(api('/api/shop-settings')).catch(() => null),
      ]);
  ...
  const calcs = useMemo(() => {
    ...
    const perEntity = entities.map(e => {
      ...
      // ✅ Absent deduction — current month, staff only
      ...
      if (e.type === 'staff' && e.active && e.monthlyAmount > 0) {
        const attKey = Object.keys(attendance).find(k =>
          k === String(e.name || '').toLowerCase().trim() ||
          k === String(e.staffId || '').toLowerCase().trim()
        );
        ...
        for (let d = 1; d <= now.getDate(); d++) {
          if (new Date(curY, curM, d).getDay() !== 0) elapsed++;
        }
        presentDays = attList.filter(a => {
          const d = new Date(a.date);
          return d.getMonth() === curM && d.getFullYear() === curY;
        }).length;
        absentDays = Math.max(0, elapsed - presentDays);
        const rulesStart = shopSettings?.attendanceRulesStartDate;
        const rulesActive = rulesStart
          ? new Date(curY, curM, 1) >= new Date(new Date(rulesStart).getFullYear(), new Date(rulesStart).getMonth(), 1)
          : false;
        if (rulesActive && absentDays > 0) {
          const perDay = workingDays > 0 ? e.monthlyAmount / workingDays : 0;
          const lateDays = attList.filter(a => {
            const d = new Date(a.date);
            if (d.getMonth() !== curM || d.getFullYear() !== curY) return false;
            const [h] = (a.checkInTime || '').split(':').map(Number);
            return h > 10;
          }).length;
          absentDeduction = Math.round(absentDays * perDay) + lateDays * (shopSettings?.latePenalty || 50);
          deductionNote = `🚫 ${absentDays}d absent · ${lateDays}d late`;
        } else if (!rulesActive) {
          deductionNote = '🕊️ Grace period — rules not active';
        } else {
          deductionNote = `✅ ${presentDays} दिन उपस्थित — कोई कटौती नहीं`;
        }
...
function EntityCard({ entity: e, onClick }) {
  ...
  return (
    <div onClick={onClick}
      ...
      {/* ✅ Absent deduction + net payable — staff only */}
      {e.type === 'staff' && (
        <div style={{ marginTop:8, background: e.absentDeduction > 0 ? '#7f1d1d22' : '#14532d22', border:`1px solid ${e.absentDeduction > 0 ? '#ef444455' : '#22c55e55'}`, borderRadius:8, padding:'7px 10px' }}>
          {e.absentDeduction > 0 ? (
            <>
              <p style={{ color:'#fca5a5', fontSize:10, fontWeight:700, margin:0 }}>✂️ कटौती: {fmtINR(e.absentDeduction)} · {e.deductionNote}</p>
              <p style={{ color:'#fbbf24', fontSize:11, fontWeight:800, margin:'4px 0 0' }}>💰 Net Payable: {fmtINR(e.netPayable)}</p>
            </>
          ) : (
            <p style={{ color:'#86efac', fontSize:10, margin:0 }}>{e.deductionNote || `💰 Net Payable: ${fmtINR(e.netPayable)}`}</p>
          )}
        </div>
      )}
...
function EntityDetails({ entity: e, onAddPayment, onEdit, onDelete, onDeletePayment, onEditPayment }) {
  ...
  return (
    <div style={{ display:'grid', gap:14 }}>
      ...
      {e.type === 'staff' && (
        <Panel title="📅 Monthly Salary Breakdown" subtitle="हर महीने की attendance, deduction, और payment">
            ...
            <table style={{ width:'100%', fontSize:11, borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ color:'#94a3b8', fontSize:10, textTransform:'uppercase', borderBottom:'1px solid #334155' }}>
                  ...
                  <th style={{ padding:'8px 6px', textAlign:'center' }}>Monthly</th>
              ...
              <tbody>
                {(() => {
                  // Build month list from start to now
                  const rows = [];
                  ...
                  while (cur <= last) {
                    ...
                    const monthPays = e.payments.filter(p => {
                      const d = new Date(p.paymentDate);
                      return d.getMonth() === m && d.getFullYear() === y && !p.cancelled;
                    });
                    const paid = monthPays.reduce((s, p) => s + (p.amount || 0), 0);
                    const monthly = e.monthlyAmount || 0;
                    const diff = paid - monthly;
                    const isCurrentMonth = (y === new Date().getFullYear() && m === new Date().getMonth());
                    const absDed = isCurrentMonth ? (e.absentDeduction || 0) : 0;
                    const netDue = Math.max(0, monthly - absDed);
                    const status = paid >= netDue ? '✅ Paid' : paid > 0 ? `⚠ Partial (${fmtINR(netDue - paid)} due)` : `❌ Due (${fmtINR(netDue)})`;
                    const statusColor = paid >= netDue ? '#86efac' : paid > 0 ? '#fbbf24' : '#fca5a5';
                    rows.push(
                      <tr key={`${y}-${m}`} style={{ borderBottom:'1px solid #1e293b', background: isCurrentMonth ? '#fbbf2408' : 'transparent' }}>
                        <td style={{ padding:'8px 6px', color: isCurrentMonth ? '#fbbf24' : '#e2e8f0', fontWeight: isCurrentMonth ? 700 : 400 }}>{label}{isCurrentMonth ? ' 🔵' : ''}</td>
                        <td style={{ padding:'8px 6px', textAlign:'center', color:'#94a3b8' }}>
                          {fmtINR(monthly)}
                          {absDed > 0 && <span style={{ display:'block', color:'#fca5a5', fontSize:9 }}>-{fmtINR(absDed)} deduction</span>}
                        </td>
                        <td style={{ padding:'8px 6px', textAlign:'center', color:'#86efac', fontWeight:700 }}>{paid > 0 ? fmtINR(paid) : '—'}</td>
                        <td style={{ padding:'8px 6px', textAlign:'center', color:statusColor, fontSize:10, fontWeight:600 }}>{status}</td>
                      </tr>
                    );
