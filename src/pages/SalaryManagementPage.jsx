            ...
            <table style={{ width:'100%', fontSize:11, borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ color:'#94a3b8', fontSize:10, textTransform:'uppercase', borderBottom:'1px solid #334155' }}>
                  ...
                  <th style={{ padding:'8px 6px', textAlign:'right' }}>Advance</th>
                  <th style={{ padding:'8px 6px', textAlign:'right' }}>Bonus</th>
                  <th style={{ padding:'8px 6px', textAlign:'right' }}>Incentive</th>
              ...
              <tbody>
                {(() => {
                  ...
                  while (cur <= last) {
                    ...
                    const totalPaid = salary+advance+bonus+incentive;
                    const monthly   = e.monthlyAmount || 0;
                    const netDue    = Math.max(0, monthly - deduction);
                    const isCur     = y===new Date().getFullYear() && m===new Date().getMonth();
                    const statusColor = totalPaid>=netDue?'#86efac':totalPaid>0?'#fbbf24':'#fca5a5';
                    const statusText  = totalPaid>=netDue?'✅ Paid':totalPaid>0?`⚠ ₹${(netDue-totalPaid).toLocaleString('en-IN')} due`:`❌ ₹${netDue.toLocaleString('en-IN')} due`;
                    rows.push(
                      <tr key={`${y}-${m}`} style={{ borderBottom:'1px solid #1e293b', background:isCur?'#fbbf2408':'transparent' }}>
                        <td style={{ padding:'7px 6px', color:isCur?'#fbbf24':'#e2e8f0', fontWeight:isCur?700:400 }}>{label}{isCur?' 🔵':''}</td>
                        ...
                        <td style={{ padding:'7px 6px', textAlign:'right', color:'#fdba74' }}>{advance>0?fmtINR(advance):'—'}</td>
                        <td style={{ padding:'7px 6px', textAlign:'right', color:'#93c5fd' }}>{bonus>0?fmtINR(bonus):'—'}</td>
                        <td style={{ padding:'7px 6px', textAlign:'right', color:'#d8b4fe' }}>{incentive>0?fmtINR(incentive):'—'}</td>
                        <td style={{ padding:'7px 6px', textAlign:'right', color:'#fca5a5' }}>{deduction>0?`-${fmtINR(deduction)}`:'—'}</td>
                        <td style={{ padding:'7px 6px', textAlign:'center', color:statusColor, fontSize:10, fontWeight:600 }}>{statusText}</td>
                      </tr>
                    );
