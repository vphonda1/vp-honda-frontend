...
export default function SalaryManagementPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // ✅ Attendance — staffId से group करें
      if (attRes && attRes.ok) {
        const attData = await attRes.json();
        const byStaff = {};
        (Array.isArray(attData) ? attData : []).forEach(a => {
          const sid = a.staffId || a.staffName;
      ...
      // ✅ Shop settings (latePenalty, attendanceRulesStartDate)
  ...
  const calcs = useMemo(() => {
    ...
    const countSundays = (y, m) => {
      ...
      while (d.getMonth() === m) { if (d.getDay() === 0) count++; d.setDate(d.getDate()+1); }
      return count;
    };
    ...
    const perEntity = entities.map(e => {
      ...
      // ✅ Absent deduction — only for staff type, current month
      let absentDeduction = 0, absentDays = 0, deductionNote = '';
      if (e.type === 'staff' && e.active && e.monthlyAmount > 0) {
        // Find attendance by entity name or staffId
        const attKey = Object.keys(attendance).find(k =>
          String(k).toLowerCase().trim() === String(e.name || '').toLowerCase().trim() ||
          String(k).toLowerCase().trim() === String(e.staffId || '').toLowerCase().trim()
        );
        ...
        // Elapsed working days so far this month
        let elapsed = 0;
        ...
        const presentDays = attList.filter(a => {
        ...
        if (rulesActive && absentDays > 0) {
          ...
          absentDeduction = Math.round(absentDays * perDay);
          const lateDays = attList.filter(a => {
            const d = new Date(a.date);
            if (d.getMonth() !== curM || d.getFullYear() !== curY) return false;
            const t = a.checkInTime || '';
            const [h, min] = t.split(':').map(Number);
            return h > 10 || (h === 10 && min > 0);
          }).length;
          const latePenalty = lateDays * (shopSettings?.latePenalty || 50);
          absentDeduction += latePenalty;
          deductionNote = `🚫 ${absentDays}d absent × ₹${Math.round(perDay)}/d${lateDays > 0 ? ` + ${lateDays}d late × ₹${shopSettings?.latePenalty||50}` : ''}`;
        } else if (!rulesActive) {
          deductionNote = '🕊️ Grace period';
        } else {
          deductionNote = `✅ पूरे ${elapsed} days उपस्थित`;
        }
      }

      return { ...e, payments: pays, totalPaid, monthsActive, expectedTotal, balance, absentDeduction, absentDays, deductionNote };
    });
...
function EntityCard({ entity: e, onClick }) {
  ...
  return (
    <div onClick={onClick}
      ...
      {e.type === 'staff' && e.absentDeduction > 0 && (
        <div style={{ marginTop:8, background:'#7f1d1d22', border:'1px solid #ef444455', borderRadius:8, padding:'6px 10px' }}>
          <p style={{ color:'#fca5a5', fontSize:10, fontWeight:700, margin:0 }}>
            ✂️ इस महीने कटौती: {fmtINR(e.absentDeduction)}
          </p>
          <p style={{ color:'#94a3b8', fontSize:9, margin:'2px 0 0' }}>{e.deductionNote}</p>
          <p style={{ color:'#fbbf24', fontSize:10, fontWeight:700, margin:'4px 0 0' }}>
            💰 Net Payable: {fmtINR(Math.max(0, (e.monthlyAmount || 0) - e.absentDeduction))}
          </p>
        </div>
      )}
      {e.type === 'staff' && e.absentDeduction === 0 && e.deductionNote && (
        <p style={{ color:'#64748b', fontSize:9, textAlign:'center', marginTop:6 }}>{e.deductionNote}</p>
      )}
