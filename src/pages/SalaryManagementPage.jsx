...
// ✅ EntityDetails को generateSalarySlip pass करने के लिए wrapper
function EntityDetailsWithSlip({ e, ...props }) {
  const generateSalarySlip = (entity) => {
    const now = new Date();
    const month = now.toLocaleString('en-IN', { month:'long', year:'numeric' });
    const thisMonthPays = entity.payments.filter(p => {
      const d = new Date(p.paymentDate);
      return d.getMonth()===now.getMonth() && d.getFullYear()===now.getFullYear() && !p.cancelled;
    });
    const salary    = thisMonthPays.filter(p=>p.type==='salary'||!p.type).reduce((s,p)=>s+p.amount,0);
    const advance   = thisMonthPays.filter(p=>p.type==='advance').reduce((s,p)=>s+p.amount,0);
    const bonus     = thisMonthPays.filter(p=>p.type==='bonus').reduce((s,p)=>s+p.amount,0);
    const incentive = thisMonthPays.filter(p=>p.type==='incentive').reduce((s,p)=>s+p.amount,0);
    const deduction = thisMonthPays.filter(p=>p.type==='deduction').reduce((s,p)=>s+p.amount,0);
    const net = salary + bonus + incentive - deduction;

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
    <title>Salary Slip</title>
    <style>
      body{font-family:Arial,sans-serif;padding:24px;color:#000;max-width:500px;margin:0 auto}
      .brand{text-align:center;border-bottom:3px solid #DC0000;padding-bottom:12px;margin-bottom:14px}
      .brand h1{color:#DC0000;margin:0;font-size:22px}
      .brand p{margin:2px 0;font-size:11px;color:#555}
      .row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px dashed #ddd;font-size:13px}
      .row.head{font-weight:bold;background:#f5f5f5;padding:8px;margin-bottom:4px}
      .row.total{font-weight:bold;font-size:15px;border-top:2px solid #DC0000;border-bottom:2px solid #DC0000;padding:10px;color:#DC0000}
      .badge{background:#DC0000;color:#fff;padding:2px 12px;border-radius:12px;font-size:11px;display:inline-block;margin-bottom:8px}
    </style></head><body>
    <div class="brand">
      <h1>🏍️ VP Honda</h1>
      <p>Parwaliya Sadak, Bhopal (M.P.) · 📞 9713394738</p>
      <p>GSTIN: 23BCYPD9538B1ZG</p>
    </div>
    <div class="badge">SALARY SLIP — ${month}</div>
    <div class="row head"><span>Employee</span><span>${entity.name}</span></div>
    <div class="row"><span>Monthly Package</span><span>₹${(entity.monthlyAmount||0).toLocaleString('en-IN')}</span></div>
    <div class="row"><span>Salary Paid</span><span style="color:green">+ ₹${salary.toLocaleString('en-IN')}</span></div>
    ${advance>0?`<div class="row"><span>Advance</span><span style="color:#f97316">+ ₹${advance.toLocaleString('en-IN')}</span></div>`:''}
    ${bonus>0?`<div class="row"><span>Bonus</span><span style="color:#3b82f6">+ ₹${bonus.toLocaleString('en-IN')}</span></div>`:''}
    ${incentive>0?`<div class="row"><span>Incentive/Commission</span><span style="color:#a855f7">+ ₹${incentive.toLocaleString('en-IN')}</span></div>`:''}
    ${deduction>0?`<div class="row"><span>Deductions</span><span style="color:#ef4444">- ₹${deduction.toLocaleString('en-IN')}</span></div>`:''}
    <div class="row total"><span>Net Payable</span><span>₹${net.toLocaleString('en-IN')}</span></div>
    <div style="margin-top:16px;font-size:11px;color:#666">
      <div style="display:flex;justify-content:space-between;margin-top:20px">
        <div>Employee Signature: ______________</div>
        <div>For: VP Honda<br/>Authorised Sign</div>
      </div>
    </div>
    </body></html>`;
  ...
  return <EntityDetails e={e} generateSalarySlip={generateSalarySlip} {...props}/>;
}
