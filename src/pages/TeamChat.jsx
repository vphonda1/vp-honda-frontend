...
import { visibleInterval, isChatOn, setChatOn, CHAT_POLL_MS, CHAT_IDLE_OFF_MS } from '../utils/pollControl';
...
export default function TeamChat({ user }) {
  // ════════════════════════════════════════════════════════════════════════
  // ⭐ Chat का master switch — Render का कोटा बचाने के लिए
  // ════════════════════════════════════════════════════════════════════════
  // पहले chat हर 3 सेकंड server से पूछता था — रोज़ 26,400 request. एक tab
  // chat पर खुला छूट जाए तो Render रातभर जागी रहती और घंटे कटते रहते.
  //
  // अब chat **बंद रहता है**. बटन दबाकर चालू कीजिए, काम ख़त्म होते ही बंद.
  // बंद हालत में एक भी request नहीं जाती.
  const [chatOn,       setChatOnState]  = useState(isChatOn);
  ...
  const [deleteTarget, setDeleteTarget] = useState(null);  // delete modal के लिए
  ...
  const loadMessages = useCallback(async (initial = false) => {
    ...
    try {
      ...
      if (!initial && lastSyncRef.current) url += `?since=${encodeURIComponent(lastSyncRef.current)}`;
      const res = await fetch(url);
      if (!res.ok) return;
      const data = await res.json();
      const hidden = getLS('vp_chat_hidden', []);

      if (initial) {
        setMessages(data.filter(m => !hidden.includes(m._id)));
        ...
        setMessages(prev => {
          ...
          data.forEach(m => {
            if (hidden.includes(m._id)) return;
            if (!map.has(m._id) && m.sender !== myName && !m.deleted) fromOthers.push(m);
            map.set(m._id, m);  // नया add या existing update (deleted/edited)
          });
        ...
        if (fromOthers.length > 0) {
          lastActivityRef.current = Date.now();   // बातचीत चल रही है — घड़ी रीसेट
          try {
  ...
  useEffect(() => {
    if (!currentRoom || !chatOn) return;   // ⬅️ बंद हो तो एक भी request नहीं
    setMessages([]); lastIdRef.current = null; lastSyncRef.current = null;
    loadMessages(true);
    setUnread(prev => { const u = { ...prev, [currentRoom]: 0 }; setLS(LS_UNREAD, u); return u; });
    lastActivityRef.current = Date.now();

    // 3s → 25s, और tab पीछे जाते ही पूरी तरह रुक जाता है.
    // साथ में: 10 मिनट कोई नया message न आए तो chat अपने आप बंद हो जाता है —
    // ताकि tab खुला भूल जाने पर भी Render सो सके.
    return visibleInterval(() => {
      if (Date.now() - lastActivityRef.current > CHAT_IDLE_OFF_MS) {
        setChatOn(false);
        return;
      }
      loadMessages(false);
    }, CHAT_POLL_MS);
  }, [currentRoom, chatOn, loadMessages]);

  // switch बदले तो तुरंत पता चले (दूसरी जगह से बंद/चालू हो तब भी)
  useEffect(() => {
    const onToggle = (e) => setChatOnState(!!e.detail);
    window.addEventListener('vp-chat-toggle', onToggle);
    return () => window.removeEventListener('vp-chat-toggle', onToggle);
  }, []);
  ...
  const sendMsg = async (extra = {}) => {
    ...
    lastActivityRef.current = Date.now();   // आप लिख रहे हैं — chat जगा रहे
  ...
  // ✅ Delete options: Everyone (दोनों तरफ) या Me (सिर्फ मेरे device से)
  const deleteMsg = (msg) => {
    setShowActions(null);
    setDeleteTarget(msg);   // modal खोलेगा
  };

  const doDeleteForEveryone = async (msg) => {
    setDeleteTarget(null);
    // तुरंत "deleted" दिखाएं (optimistic)
    setMessages(prev => prev.map(m => m._id === msg._id ? { ...m, deleted: true, text: '🚫 This message was deleted', fileData: null, fileType: 'text' } : m));
    try {
      await fetch(api(`/api/messages/${currentRoom}/${msg._id}`), {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sender: myName, scope: 'everyone' }),
      });
    } catch {}
  };
  ...
  const doDeleteForMe = (msg) => {
    setDeleteTarget(null);
    // सिर्फ इस device से छुपाएं (localStorage में hidden list)
    const hidden = getLS('vp_chat_hidden', []);
    hidden.push(msg._id);
    setLS('vp_chat_hidden', hidden);
    ...
    return (
      <div style={{ display:'flex', flexDirection:isMe?'row-reverse':'row', alignItems:'flex-end', gap:6 }}>
        ...
        <div style={{ maxWidth:'72%', minWidth:60 }}>
          ...
          <div onContextMenu={(e) => { e.preventDefault(); if (!msg.deleted) setShowActions(msg._id); }}
            onClick={() => { if (!msg.deleted && msg.fileType === 'image') setImageView(msg.fileData); }}
            style={{
              ...
              padding: ['image','video'].includes(msg.fileType) && !msg.deleted ? 3 : '8px 12px',
            ...
            {/* DELETED placeholder */}
            {msg.deleted ? (
              <p style={{ fontSize:12, margin:0, color:'#94a3b8', fontStyle:'italic' }}>🚫 This message was deleted</p>
            ) : (
            <>
            {/* IMAGE */}
  ...
  // ════════════════════════════════════════════════════════════════════════
  // Chat बंद है — start screen
  // ════════════════════════════════════════════════════════════════════════
  // यहाँ कोई useEffect नहीं चलता, कोई interval नहीं, कोई fetch नहीं.
  // यानी इस हालत में Render पर एक भी request नहीं जाती — पूरी तरह leak-free.
  if (!chatOn) {
    return (
      <div style={{
        height:'calc(100dvh - 48px)', background:'#020617', color:'#fff',
        ...
        <div style={{
          ...
          display:'grid', placeItems:'center', fontSize:32, marginBottom:18,
        }}>💬</div>

        <h2 style={{ fontSize:20, fontWeight:800, margin:'0 0 8px' }}>Team Chat बंद है</h2>
        <p style={{ color:'#64748b', fontSize:12.5, maxWidth:340, lineHeight:1.7, margin:'0 0 22px' }}>
          Chat चालू रहने पर server से लगातार बात होती रहती है और Render के मुफ़्त
          घंटे कटते हैं. इसलिए यह अब बंद रहता है — ज़रूरत हो तभी चालू कीजिए.
        </p>

        <button onClick={() => { setChatOn(true); setChatOnState(true); }} style={{
          background:'linear-gradient(135deg,#7c3aed,#6d28d9)', border:'none',
          borderRadius:14, padding:'14px 34px', color:'#fff',
        ...
        <div style={{
          marginTop:26, background:'rgba(255,255,255,.03)',
          border:'1px solid rgba(255,255,255,.08)', borderRadius:14,
          padding:'13px 17px', maxWidth:360, textAlign:'left',
        }}>
          <p style={{ color:'#94a3b8', fontSize:11, fontWeight:700, margin:'0 0 7px' }}>चालू करने पर:</p>
          <ul style={{ color:'#64748b', fontSize:11, margin:0, paddingLeft:17, lineHeight:1.9 }}>
            <li>पुराने सारे message वैसे ही मिलेंगे — कुछ खोता नहीं</li>
            <li>हर 25 सेकंड में नए message आते हैं</li>
            <li>दूसरी tab पर जाते ही अपने आप रुक जाता है</li>
            <li>10 मिनट कोई message न आए तो अपने आप बंद</li>
          </ul>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display:'flex', height:'calc(100dvh - 48px)', background:'#020617', color:'#fff', overflow:'hidden' }}>
      ...
      {/* ✅ Delete options modal — WhatsApp जैसा */}
      {deleteTarget && (
        <div onClick={() => setDeleteTarget(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:210, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:'#1e293b', borderRadius:14, padding:20, width:'100%', maxWidth:320, border:'1px solid #334155' }}>
            <h3 style={{ margin:'0 0 16px', fontSize:15, fontWeight:700, color:'#fff' }}>Message delete करें?</h3>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {/* Delete for Everyone — सिर्फ अपने message पर */}
              {deleteTarget.sender === myName && (
                <button onClick={() => doDeleteForEveryone(deleteTarget)} style={{ background:'#dc2626', color:'#fff', border:'none', padding:'12px', borderRadius:8, fontWeight:700, fontSize:13, cursor:'pointer', textAlign:'left' }}>
                  🗑️ Delete for Everyone
                  <div style={{ fontSize:10, opacity:0.8, fontWeight:400, marginTop:2 }}>सबके pass से हट जाएगा</div>
                </button>
              )}
              {/* Delete for Me — हमेशा */}
              <button onClick={() => doDeleteForMe(deleteTarget)} style={{ background:'#334155', color:'#fff', border:'none', padding:'12px', borderRadius:8, fontWeight:700, fontSize:13, cursor:'pointer', textAlign:'left' }}>
                🙈 Delete for Me
                <div style={{ fontSize:10, opacity:0.8, fontWeight:400, marginTop:2 }}>सिर्फ मेरे phone से हटेगा</div>
              </button>
              <button onClick={() => setDeleteTarget(null)} style={{ background:'transparent', color:'#94a3b8', border:'1px solid #334155', padding:'10px', borderRadius:8, fontWeight:600, fontSize:13, cursor:'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div style={{ padding:'12px 12px 8px', borderBottom:'1px solid #1e293b' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <div style={{ display:'flex', gap:4 }}>
              {/* ⭐ Chat बंद करने का बटन — दबाते ही सारी polling रुक जाती है */}
              <button
                onClick={() => { setChatOn(false); setChatOnState(false); }}
                title="Chat बंद करें (server से बात रुक जाएगी)"
                style={{ background:'rgba(220,38,38,.15)', border:'1px solid rgba(220,38,38,.4)',
                  borderRadius:7, color:'#fca5a5', cursor:'pointer', padding:'3px 8px',
