// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-17-BACKGROUND-FAST 🆕🆕🆕
// ✅ compress background में + सीधा render→1-bit (3 गुना तेज़) + एक ही PDF ≤200KB
// ═══════════════════════════════════════════════════════════════════════════
...
async function processFile(file, type) {
  ...
  if (type === 'pdf') {
    // ⚡⚡ compress अब *background* में चलेगी — file तुरंत लग जाती है और आप
    //     बाकी fields (expiry date, notes) भरते रहिए। Save दबाने तक अक्सर
    //     compress पूरी हो चुकी होती है, यानी इंतज़ार लगभग शून्य।
      ...
      compressPromise = (async () => {
        try {
          ...
          if (res.files?.[0] && res.files[0].size < file.size) {
            const f = new File([res.files[0]], file.name, { type: 'application/pdf' });
            showInAppToast('✅ PDF छोटी हुई',
              `${Math.round(file.size/1024)}KB → ${Math.round(f.size/1024)}KB`, 'success');
            return { file: f, note: res.note };
          }
        } catch (e) {
          console.warn('[background compress]', e?.message);
        }
        return null;
      })();
    }
...
function cloudinaryPageUrl(url, page, width = 1500) {
...
async function compressExistingPdf(bytes, d, onProgress, opts = {}) {
  ...
  // ⚡ सिर्फ 2 कोशिश — हर कोशिश में पूरी PDF दोबारा खुलती-सिमटती है (भारी काम)।
  //    Console ने दिखाया कि 200px तक जाने पर भी 236KB पर अटक जाती है, यानी बचा
  //    हुआ वज़न text/fonts का है, images का नहीं — इसलिए 6 बार पीसना बेकार था।
  //    ⚡ अब सिर्फ 1 कोशिश — दूसरी में सिर्फ ~50KB और घटती थी, 200KB तक कभी नहीं
  const imgSteps = [{ side: 800, q: 0.52 }];
  try {
  ...
  try {
    let bw = null;
    // ⚡ पहले सीधा रास्ता: pdf.js से render → तुरंत 1-bit (कोई JPEG बीच में नहीं)
    try {
      bw = await bilevelFromPdfBytes(bytes, LIMIT, onProgress);
    } catch (eDirect) {
      console.warn('[bilevel direct]', eDirect?.message);
      // pdf.js न चले तो Cloudinary वाले pages से
      const { pages, via } = await getPagesAsBlobs(d, bytes, onProgress);
      cachedPages = pages; cachedVia = via;
      bw = await buildBilevelPdf(pages, LIMIT, onProgress);
    }
    if (bw) {
      ...
      if (bw.length < bytes.length) {
        return { files: [asFile(bw)], note: `⚠️ ${Math.round(bw.length/1024)}KB तक ही आई (एक ही file) — ${origKB}KB से घटी` };
      }
    }
  } catch (e) { console.warn('[bilevel]', e?.message); errors.push(`1-bit: ${e?.message}`); }

  // ── तरीका 3: JPEG raster — सिर्फ तब, जब 1-bit चला ही नहीं ──
  //    (bilevel कुछ बना पाया तो ये चलाना बेकार है — वो हमेशा बेहतर होता है)
  try {
    if (bestSoFar) throw new Error('1-bit बेहतर है');
...
export default function DocumentVault() {
  ...
  const saveDoc = async (stayOpen = false) => {
    ...
    try {
      ...
      // ⚡ background compress अभी चल रही हो तो यहीं उसका इंतज़ार (आमतौर पर तब तक हो चुकी होती है)
      let uploadBlob = fileData.blob;
      if (fileData.compressPromise) {
        if (fileData.compressing) showInAppToast('🗜️', 'PDF compress पूरी हो रही है...', 'info');
        try {
          const res = await fileData.compressPromise;
          if (res?.file) uploadBlob = res.file;
        } catch { /* original से ही चलेगा */ }
      }
  ...
  const handlePicked = async (f, type) => {
    ...
    try {
      ...
      const fd = await processFile(f, type);
      setFileData(fd);
      // background compress पूरी हो तो size अपने-आप update हो जाए
      if (fd.compressPromise) {
        fd.compressPromise.then(res => {
          if (!res?.file) { setFileData(p => (p && p.fileName === fd.fileName) ? { ...p, compressing: false } : p); return; }
          setFileData(p => (p && p.fileName === fd.fileName)
            ? { ...p, blob: res.file, sizeKB: Math.round(res.file.size / 1024), compMethod: res.note, compressing: false }
            : p);
        });
      }
    } catch (err) {
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        {fileData.fileType !== 'image' && (
                          fileData.compressing
                            ? <p style={{ fontSize:10, margin:'3px 0 0', color:'#60a5fa' }}>
                                🗜️ {fileData.sizeKB} KB · background में compress हो रही है — आप form भरते रहिए
                              </p>
                            : fileData.compMethod
                              ? <p style={{ fontSize:10, margin:'3px 0 0', color:'#86efac' }}>
                                  ✅ {fileData.origKB}KB → {fileData.sizeKB}KB
                                </p>
