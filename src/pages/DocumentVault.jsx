// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-17-FAST-BILEVEL 🆕🆕🆕
// ✅ तेज़ compress (नाप कर एक बार) + UI नहीं जमती + एक ही PDF ≤200KB
// ═══════════════════════════════════════════════════════════════════════════
    ...
    if (file.size > MAX_PDF_KB * 1024) {
      try {
        ...
        const res = await compressExistingPdf(
          ...
          (p, tot, extra) => {
            // हर 2 सेकंड में एक बार खबर — लगे नहीं कि app अटक गया
            if (Date.now() - lastShown > 2000) {
              lastShown = Date.now();
              showInAppToast('🗜️', `Page ${p}/${tot}${extra ? ' · ' + extra : ''}`, 'info');
            }
          },
...
async function compressExistingPdf(bytes, d, onProgress, opts = {}) {
  ...
  const imgSteps = [{ side: 900, q: 0.58 }, { side: 400, q: 0.40 }];
  try {
