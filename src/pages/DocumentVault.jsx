// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-PDF-200KB-HARD 🆕🆕🆕
// ✅ हर PDF ≤ 200KB (multi-page PDF भी) + pdf.js CDN fallback + one-tap share
// ═══════════════════════════════════════════════════════════════════════════
...
// ── pdf.js loader — npm पर न मिले तो CDN से लोड (package.json बदलने की ज़रूरत नहीं) ──
let _pdfjsPromise = null;
function loadPdfJs() {
  if (_pdfjsPromise) return _pdfjsPromise;
  _pdfjsPromise = (async () => {
    // 1) npm वाली pdfjs-dist (अगर install है)
    try {
      ...
      if (l?.getDocument) {
        if (l.GlobalWorkerOptions && !l.GlobalWorkerOptions.workerSrc) {
          l.GlobalWorkerOptions.workerSrc = `https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${l.version || '3.11.174'}/pdf.worker.min.js`;
        }
        console.log('[pdf.js] npm build loaded');
        return l;
      }
    } catch (e) { console.warn('[pdf.js] npm नहीं मिली:', e?.message); }

    // 2) CDN से UMD build (window.pdfjsLib)
    const VER = '3.11.174';
    const cdns = [
    ...
    for (const src of cdns) {
      try {
        await new Promise((res, rej) => {
          ...
          sc.onload = res;
          sc.onerror = () => rej(new Error('script load fail'));
        ...
        if (window.pdfjsLib?.getDocument) {
          ...
          console.log('[pdf.js] CDN build loaded');
          return window.pdfjsLib;
        }
      } catch (e) { console.warn('[pdf.js CDN]', src, e?.message); }
    }
    throw new Error('pdf.js load नहीं हुई');
  })();
...
// ── मौजूदा PDF को 200KB के अंदर लाएं — हर page अलग से budget में ─────────────
async function compressExistingPdf(bytes, d, onProgress) {
  if (bytes.length <= MAX_PDF_KB * 1024) {
    return new File([bytes], safeName(d, '.pdf'), { type: 'application/pdf' });
  }
  try {
    ...
    // ✅ हर page का हिस्सा — 4 page हैं तो हर page ~46KB
    ...
    for (let p = 1; p <= pageCount; p++) {
      ...
      // ऊँचे resolution पर render → अक्षर की धार बनी रहती है
      const base = page.getViewport({ scale: 1 });
      const targetW = pageCount > 6 ? 1400 : 1750;      // ज़्यादा pages हों तो थोड़ा कम
      ...
      console.log(`[PDF p${p}] ${enc.tag} → ${Math.round(enc.blob.size/1024)}KB (budget ${Math.round(perPage/1024)}KB)`);

      // page का असली अनुपात रखें — कुछ कटेगा या खिंचेगा नहीं
      const pgW = 595, pgH = Math.round(595 * (ch / cw));
      const pg = out.addPage([pgW, Math.min(Math.max(pgH, 200), 1400)]);
      pg.drawImage(img, { x: 0, y: 0, width: pg.getWidth(), height: pg.getHeight() });
    }
    await src.destroy();

    const outBytes = await out.save({ useObjectStreams: true });
    console.log(`[PDF-shrink ${d.docTypeLabel}] ${Math.round(bytes.length/1024)}KB → ${Math.round(outBytes.length/1024)}KB`);
    if (outBytes.length < bytes.length) {
      return new File([outBytes], safeName(d, '.pdf'), { type: 'application/pdf' });
    }
  } catch (e) {
    console.warn('[compressExistingPdf] fail:', e?.message);
    showInAppToast('⚠️ PDF compress fail', `${d.docTypeLabel} — original भेजी जाएगी`, 'warning');
  }
  // Fallback — original PDF (कम से कम document मिल तो जाए)
  return new File([bytes], safeName(d, '.pdf'), { type: 'application/pdf' });
}
  ...
  for (const d of valid) {
    ...
    try {
      if (d.fileType === 'pdf') {
        ...
        files.push(await compressExistingPdf(bytes, d, (p, tot) =>
...
export default function DocumentVault() {
  ...
  const prepareShare = async (validDocs, textMsg, title) => {
    ...
    try {
      ...
      setSharePack({ files, fileLabels, failReasons, totalKB, textMsg, title });
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {sharePack.failReasons.length > 0 && (
              <div style={{ background:'#422006', border:'1px solid #854d0e', borderRadius:8, padding:'8px 10px', marginBottom:10 }}>
                <p style={{ margin:0, fontSize:10, color:'#fbbf24' }}>⚠️ {sharePack.failReasons.length} document तैयार नहीं हुए: {sharePack.failReasons.slice(0,3).join(' · ')}</p>
              </div>
            )}
