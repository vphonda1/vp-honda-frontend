// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-CLOUDINARY-SHARE-FIX 🆕🆕🆕
// ✅ इस version में: upload memory fix + Share/PDF का Cloudinary URL fix
// ═══════════════════════════════════════════════════════════════════════════
...
async function imageToPDF(d) {
  ...
  const { bytes: imgBytes, mime } = await getDocBytes(d);
...
// ── Share — हर doc अलग PDF, सब एक साथ multi-share ──────────────────────────
async function shareFilesAuto(docs, textMsg, title) {
  // ✅ fileUrl (Cloudinary) OR fileData (base64) — दोनों valid
  ...
  for (const d of valid) {
    try {
      if (d.fileType === 'pdf') {
        // ✅ Existing PDF — Cloudinary URL या base64, दोनों से bytes लेकर file बनाएं
        ...
        else throw new Error('PDF बना नहीं');
      } else {
        // ✅ Video — URL या base64
  ...
  if (files.length === 0) {
    // ✅ अब असली कारण दिखेगा (pdf-lib का blanket message नहीं)
    alert(`❌ कोई भी document तैयार नहीं हो पाया।\n\n${failReasons.slice(0, 6).join('\n')}\n\nInternet check करें और दोबारा try करें।`);
    return false;
  }
  if (failReasons.length > 0) {
    showInAppToast('⚠️', `${failReasons.length} document skip हुए — ${files.length} भेज रहे हैं`, 'warning');
  }
...
export default function DocumentVault() {
  ...
  const sendSUTax = async (folderDocs, folder) => {
    ...
    await shareFilesAuto(valid, msg, `SU Tax - ${folder.name}`);
  };
