// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-PDF-PLAN-ABC 🆕🆕🆕
// ✅ PDF compress: Plan A (pdf.js) → B (resave) → C (page split) — कोई चुप fail नहीं
// ═══════════════════════════════════════════════════════════════════════════
...
// ── pdf.js loader ────────────────────────────────────────────────────────────
// ⚠️ पिछली बार यही fail हो रहा था: npm वाली pdfjs-dist (v4/v5) load होती थी
//    लेकिन worker पुराने version का था → "API version does not match Worker version"
//    → getDocument crash → चुपचाप original PDF चली जाती थी।
//    अब हमेशा एक ही pinned version (library + worker दोनों एक जगह से) उठाते हैं।
...
function loadPdfJs() {
  ...
  _pdfjsPromise = (async () => {
    if (window.pdfjsLib?.getDocument && window.pdfjsLib.GlobalWorkerOptions?.workerSrc) return window.pdfjsLib;
    const errs = [];
    for (const src of PDFJS_SOURCES) {
      try {
        if (!window.pdfjsLib?.getDocument) await loadScriptOnce(src.js);
        if (!window.pdfjsLib?.getDocument) throw new Error('pdfjsLib नहीं मिला');
        // ✅ worker उसी source/version से — mismatch की गुंजाइश नहीं
        window.pdfjsLib.GlobalWorkerOptions.workerSrc = src.worker;
        console.log('[pdf.js] loaded from', src.js);
    ...
    PDFJS_LOAD_ERROR = errs.join(' | ');
    _pdfjsPromise = null;   // ✅ अगली बार दोबारा try कर सके
    throw new Error(`pdf.js load fail — ${PDFJS_LOAD_ERROR}`);
  })();
...
// ── PLAN A: pdf.js से हर page render करके 200KB के अंदर लाएं ─────────────────
async function planA_raster(bytes, d, onProgress) {
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    console.log(`[PLAN-A p${p}] ${enc.tag} -> ${Math.round(enc.blob.size/1024)}KB / budget ${Math.round(perPage/1024)}KB`);
...
// ── PLAN B: बिना pdf.js — pdf-lib से दोबारा save (duplicate objects हटते हैं) ──
async function planB_resave(bytes, d) {
  const { PDFDocument } = await import('pdf-lib');
...
// ── PLAN C: page-wise अलग-अलग PDF (हर page अपनी file — size बँट जाती है) ─────
async function planC_split(bytes, d) {
  const { PDFDocument } = await import('pdf-lib');
  ...
  console.log(`[PLAN-C] ${n} pages -> ${files.map(f => Math.round(f.size/1024) + 'KB').join(', ')}`);
  return files;
}
...
async function compressExistingPdf(bytes, d, onProgress) {
  ...
  const plans = [
    { name: 'A (pdf.js render)', fn: () => planA_raster(bytes, d, onProgress) },
    { name: 'B (pdf-lib resave)', fn: () => planB_resave(bytes, d) },
    { name: 'C (page-wise split)', fn: () => planC_split(bytes, d) },
  ];
...
export default function DocumentVault() {
  ...
  const uploadToCloudinary = async (source, fileName, fileType) => {
    ...
    const doUpload = async () => {
      ...
      // ✅ PDF को 'raw' में भेजें — Cloudinary का "PDF delivery" block इन पर लागू नहीं होता
      //    (401 Download fail की दिक्कत आगे से नहीं आएगी)
