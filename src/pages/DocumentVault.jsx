// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-PDF-PLAN-ABCD 🆕🆕🆕
// ✅ PDF compress: A Cloudinary→JPG · B pdf.js · C resave · D page-split
// ═══════════════════════════════════════════════════════════════════════════
...
async function processFile(file, type) {
  ...
  // PDF / Video — base64 (compress browser में संभव नहीं)
...
// एक page के canvas को budget के अंदर encode करें — text की सफाई पहले, size बाद में
async function encodePageWithinBudget(canvas, budgetBytes) {
  const w = canvas.width, h = canvas.height;
  const ctx = canvas.getContext('2d');
  let best = null;
  const consider = (blob, type, tag) => {
    if (!blob) return false;
    if (!best || blob.size < best.blob.size) best = { blob, type, tag };
    return blob.size <= budgetBytes;
  };

  // Step 1 — रंगीन JPEG (जैसा original है)
  for (const q of [0.75, 0.62, 0.5]) {
    if (consider(await blobOf(canvas, 'image/jpeg', q), 'jpg', `color q${q}`)) return best;
  }
  // Step 2 — Grayscale (documents में रंग की ज़रूरत नहीं, size आधी हो जाती है)
  toGrayscale(ctx, w, h);
  for (const q of [0.65, 0.55, 0.45, 0.38]) {
    if (consider(await blobOf(canvas, 'image/jpeg', q), 'jpg', `gray q${q}`)) return best;
  }
  // Step 3 — Contrast boost: सफेद background पूरी सफेद, अक्षर पूरे काले
  //           → JPEG बहुत छोटी बनती है और text और भी साफ पढ़ा जाता है
  boostContrast(ctx, w, h);
  for (const q of [0.6, 0.48, 0.4, 0.32, 0.26]) {
    if (consider(await blobOf(canvas, 'image/jpeg', q), 'jpg', `bw q${q}`)) return best;
  }
  // Step 4 — PNG (साफ काले-सफेद page पर अक्सर सबसे छोटी + बिल्कुल sharp)
  consider(await blobOf(canvas, 'image/png'), 'png', 'png-bw');
  return best;
}

// ── PLAN A: Cloudinary से हर page सीधे JPG में लें (server-side, सबसे पक्का) ──
async function planA_cloudinary(d, onProgress) {
  const url = isHttpUrl(d.fileUrl) ? d.fileUrl : (isHttpUrl(d.fileData) ? d.fileData : null);
  if (!url) throw new Error('Cloudinary URL नहीं है');
  if (!cloudinaryPageUrl(url, 1)) throw new Error('raw file — transform नहीं होता');

  const { PDFDocument } = await import('pdf-lib');
  const pageBlobs = [];
  for (let p = 1; p <= 20; p++) {
    onProgress?.(p, '?');
    const pUrl = cloudinaryPageUrl(url, p);
    const res = await fetch(pUrl, { mode: 'cors', cache: 'no-store' });
    if (!res.ok) {
      if (p === 1) throw new Error(`Cloudinary convert fail (${res.status})`);
      break;                                  // pages खत्म
    }
    const blob = await res.blob();
  ...
  if (!pageBlobs.length) throw new Error('कोई page नहीं मिला');

  const perPage = Math.floor((MAX_PDF_KB * 1024 * 0.93) / pageBlobs.length);
  const out = await PDFDocument.create();
  for (let i = 0; i < pageBlobs.length; i++) {
    onProgress?.(i + 1, pageBlobs.length);
    const bitmap = await createImageBitmap(pageBlobs[i]);
    const canvas = document.createElement('canvas');
    ...
    bitmap.close();
    const enc = await encodePageWithinBudget(canvas, perPage);
    const cw = canvas.width, ch = canvas.height;
    canvas.width = 0; canvas.height = 0;
    if (!enc) continue;
    const imgBytes = new Uint8Array(await enc.blob.arrayBuffer());
    const img = enc.type === 'png' ? await out.embedPng(imgBytes) : await out.embedJpg(imgBytes);
    console.log(`[PLAN-A cloudinary p${i+1}] ${enc.tag} -> ${Math.round(enc.blob.size/1024)}KB`);
  ...
  if (!outBytes.length) throw new Error('खाली PDF बनी');
  return [new File([outBytes], safeName(d, '.pdf'), { type: 'application/pdf' })];
}

// ── PLAN B: pdf.js से हर page render करके 200KB के अंदर लाएं ─────────────────
async function planB_raster(bytes, d, onProgress) {
  const pdfjs = await loadPdfJs();
  const { PDFDocument } = await import('pdf-lib');

  const src = await pdfjs.getDocument({ data: bytes.slice(0), disableFontFace: false }).promise;
  const pageCount = Math.min(src.numPages, 30);
  const perPage = Math.floor((MAX_PDF_KB * 1024 * 0.93) / pageCount);
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    const targetW = pageCount > 6 ? 1400 : 1750;
    const scale = Math.min(2.6, Math.max(1.1, targetW / base.width));
    ...
    const imgBytes = new Uint8Array(await enc.blob.arrayBuffer());
    const img = enc.type === 'png' ? await out.embedPng(imgBytes) : await out.embedJpg(imgBytes);
    console.log(`[PLAN-B p${p}] ${enc.tag} -> ${Math.round(enc.blob.size/1024)}KB / budget ${Math.round(perPage/1024)}KB`);
    const pgH = Math.min(Math.max(Math.round(595 * (ch / cw)), 200), 1400);
    const pg = out.addPage([595, pgH]);
    pg.drawImage(img, { x: 0, y: 0, width: 595, height: pgH });
  }
...
// ── PLAN C: बिना pdf.js — pdf-lib से दोबारा save (duplicate objects हटते हैं) ──
async function planC_resave(bytes, d) {
  const { PDFDocument } = await import('pdf-lib');
  const doc = await PDFDocument.load(bytes, { ignoreEncryption: true, updateMetadata: false });
  const outBytes = await doc.save({ useObjectStreams: true, addDefaultPage: false });
  if (outBytes.length >= bytes.length) throw new Error('resave से फायदा नहीं');
  return [new File([outBytes], safeName(d, '.pdf'), { type: 'application/pdf' })];
}

// ── PLAN D: page-wise अलग-अलग PDF (हर page अपनी file — size बँट जाती है) ─────
async function planD_split(bytes, d) {
  const { PDFDocument } = await import('pdf-lib');
  const doc = await PDFDocument.load(bytes, { ignoreEncryption: true });
  const n = doc.getPageCount();
  if (n < 2) throw new Error('single page — split नहीं हो सकती');
  const files = [];
  for (let i = 0; i < Math.min(n, 20); i++) {
    const one = await PDFDocument.create();
    const [pg] = await one.copyPages(doc, [i]);
    one.addPage(pg);
    const b = await one.save({ useObjectStreams: true });
    files.push(new File([b], safeName(d, `_p${i + 1}.pdf`), { type: 'application/pdf' }));
  }
  console.log(`[PLAN-D] ${n} pages -> ${files.map(f => Math.round(f.size/1024) + 'KB').join(', ')}`);
  return files;
}

// ── मौजूदा PDF को छोटा करें — A fail हो तो B, B fail हो तो C ────────────────
//    हर हाल में document भेजा जाएगा, चुपचाप fail नहीं होगा
async function compressExistingPdf(bytes, d, onProgress) {
  const origKB = Math.round(bytes.length / 1024);
  if (bytes.length <= MAX_PDF_KB * 1024) {
    return { files: [new File([bytes], safeName(d, '.pdf'), { type: 'application/pdf' })], note: null };
  }

  const plans = [
    { name: 'A (Cloudinary→JPG)', fn: () => planA_cloudinary(d, onProgress) },
    { name: 'B (pdf.js render)',  fn: () => planB_raster(bytes, d, onProgress) },
    { name: 'C (pdf-lib resave)', fn: () => planC_resave(bytes, d) },
    { name: 'D (page-wise split)',fn: () => planD_split(bytes, d) },
  ];

  const errors = [];
  for (const plan of plans) {
    try {
      const files = await plan.fn();
      const biggest = Math.max(...files.map(f => f.size));
      const totalKB = Math.round(files.reduce((a, f) => a + f.size, 0) / 1024);
      console.log(`[${d.docTypeLabel}] Plan ${plan.name}: ${origKB}KB -> ${totalKB}KB (सबसे बड़ी ${Math.round(biggest/1024)}KB)`);
      if (biggest <= MAX_PDF_KB * 1024) {
        showInAppToast('🗜️ Compress हुआ', `${d.docTypeLabel}: ${origKB}KB → ${totalKB}KB · Plan ${plan.name}`, 'success');
        return { files, note: `${plan.name} से ${origKB}KB → ${totalKB}KB` };
      }
      errors.push(`${plan.name}: ${Math.round(biggest/1024)}KB तक ही आई`);
    } catch (e) {
      console.warn(`[Plan ${plan.name} fail]`, e?.message);
      errors.push(`${plan.name}: ${e?.message || 'fail'}`);
    }
  }

  // सब fail — original भेजें, पर साफ बताएं कि क्यों
  return {
    files: [new File([bytes], safeName(d, '.pdf'), { type: 'application/pdf' })],
    note: `⚠️ ${d.docTypeLabel} ${origKB}KB ही रह गई — ${errors.join(' · ')}`,
  };
}
...
export default function DocumentVault() {
  ...
  const uploadToCloudinary = async (source, fileName, fileType) => {
    ...
    const doUpload = async () => {
      ...
      // ✅ 'auto' ही रखें — इससे PDF, Cloudinary पर image बनकर जाती है और
      //    server-side page→JPG conversion (Plan A) चल पाता है।
      //    ('raw' में transformation नहीं होता, इसलिए compress भी नहीं हो पाती)
      ...
      if (!r.ok) {
        // ✅ असली कारण दिखेगा (जैसे: File size too large / Upload preset not found)
    ...
    try {
      ...
      return await doUpload(); // 1 retry (network glitch के लिए)
    }
  };
