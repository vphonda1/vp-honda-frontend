...
// ── Share — हर document अलग PDF, सब एक साथ share ─────────────────────────────
async function shareFilesAuto(docs, textMsg, title) {
  ...
  showInAppToast('🔄', `${valid.length} PDFs तैयार हो रही हैं...`, 'info');
  const files = [];
  for (const d of valid) {
    try {
      if (d.fileType === 'pdf') {
        // Existing PDF — as-is रखें (multi-page हो तो भी)
  ...
  // ✅ STEP 2: सब PDFs एक साथ share (अलग-अलग files रहेंगी, merge नहीं)
  if (navigator.share) {
    ...
    // Fallback: one-by-one (अगर multi-PDF share fail हुआ)
    try {
      await navigator.share({ title: files[0].name, text: textMsg, files: [files[0]] });
      for (let i = 1; i < files.length; i++) {
        await new Promise(r => setTimeout(r, 1200));
        try {
          await navigator.share({
            title: files[i].name,
            text: `📄 ${valid[i]?.docTypeLabel || ''} (${i+1}/${files.length})`,
            files: [files[i]],
          });
  ...
  // Desktop fallback: सब PDFs download + WhatsApp text
  ...
  if (phone) sendWhatsApp(phone, textMsg + '\n\n📁 PDFs download हो गई — WhatsApp में attach करें');
  return true;
}

// Backup functions (अब use नहीं हो रहे)
