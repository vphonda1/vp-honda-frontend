...
async function shareFilesAuto(docs, textMsg, title) {
  ...
  // ✅ अगर एक भी file नहीं है — error दिखाएं
  if (files.length === 0) {
    alert('❌ Documents load नहीं हुए। Network slow है या files corrupt हैं।\n\nफिर try करें।');
    return false;
  }
  ...
  if (navigator.share) {
    try {
      // ✅ FIRST: सब files एक साथ try करें
      if (navigator.canShare && navigator.canShare({ files })) {
        await navigator.share({ title, text: textMsg, files });
        return true;
      }

      // ✅ SECOND: एक-एक file try करें (mobile पर large files के लिए)
      alert(`${files.length} files एक साथ share नहीं हो सकती। एक-एक करके भेज रहे हैं...`);

      // First share with full message
      await navigator.share({ title: `${title} - ${files[0].name}`, text: textMsg, files: [files[0]] });

      // Rest share with file index
      for (let i = 1; i < files.length; i++) {
        ...
        try {
          ...
          console.warn(`File ${i+1} share failed:`, e.message);
        }
      }
      return true;
    } catch (e) {
      ...
      alert(`❌ Share error: ${e.message}\n\nWhatsApp manually open करके भेजें।`);
    }
  }
