// DocumentVault.jsx — VP Honda (Hypothecation + WhatsApp Multi-File Send via Backend)
...
// Helper: send files + text via backend (multi-file WhatsApp)
async function sendViaBackend(phoneNumber, docsArray, caption) {
    ...
    try {
        const res = await fetch(api('/api/send-whatsapp-multi'), { method: 'POST', body: formData });
        const data = await res.json();
        if (data.success) alert('✅ सभी डॉक्यूमेंट WhatsApp पर भेज दिए गए!');
        else alert('❌ भेजने में त्रुटि: ' + data.error);
    } catch (err) {
        alert('बैकएंड से कनेक्ट नहीं हो पाया। कृपया सुनिश्चित करें बैकएंड चालू है।');
    }
}
...
export default function DocumentVault() {
    ...
    const sendInsurance = async (folderDocs, folder) => {
        ...
        if (available.length === 0) {
            alert('इंश्योरेंस के लिए कोई डॉक्यूमेंट उपलब्ध नहीं।\nजरूरी: VP Tax Invoice, Aadhar, PAN, Challan, Chassis Trace');
            return;
        }
        ...
        let phone = prompt('📞 Insurance WhatsApp नंबर (ग्रुप या व्यक्ति) देश कोड सहित (जैसे 919876543210):', '');
        if (!phone) return;
        phone = phone.replace(/\D/g, '');
        if (!phone.startsWith('91')) phone = '91' + phone;
        await sendViaBackend(phone, available, textMsg);
    };
    ...
    const shareSingleDoc = async (doc) => {
        ...
        phone = phone.replace(/\D/g, '');
        if (!phone.startsWith('91')) phone = '91' + phone;
        await sendViaBackend(phone, [doc], textMsg);
    };
