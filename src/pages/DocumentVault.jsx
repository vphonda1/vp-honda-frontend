...
async function shareFilesAuto(docs, textMsg, title) {
  ...
  if (navigator.share) {
    ...
    // Multiple files: try all together FIRST
    ...
    // Fallback: one-by-one (each file with full text message)
    // यह तब चलता है जब WhatsApp multi-file accept नहीं करता
    try {
      // पहली file में full text
      await navigator.share({ title: files[0].name, text: textMsg, files: [files[0]] });
      // बाकी files में short caption
      for (let i = 1; i < files.length; i++) {
        await new Promise(r => setTimeout(r, 1200));
        try {
          await navigator.share({
            title: files[i].name,
            text: `📎 ${docs[i]?.docTypeLabel || files[i].name} (${i+1}/${files.length})`,
            files: [files[i]],
          });
