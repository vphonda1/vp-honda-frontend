...
// ── Share files — Multi-file first, individual fallback (NO POPUPS) ───────────
async function shareFilesAuto(docs, textMsg, title) {
  ...
  // ✅ Detect mixed types (images + PDFs together) — WhatsApp इन्हें एक साथ accept नहीं करता
  const hasPDF    = docs.some(d => d.fileType === 'pdf');
  const hasVideo  = docs.some(d => d.fileType === 'video');
  const hasImage  = docs.some(d => d.fileType !== 'pdf' && d.fileType !== 'video');
  const isMixed   = (hasPDF && hasImage) || (hasVideo && hasImage) || (hasPDF && hasVideo);

  // ✅ Web Share API
  if (navigator.share) {
    // Single file: simplest case — share with text caption
    if (files.length === 1) {
      try {
    ...
    // ✅ Multiple files of SAME type: try all together first
    // Mixed types (PDF+image, etc): skip multi-file, go straight to one-by-one
    if (!isMixed && navigator.canShare && navigator.canShare({ files })) {
      try {
        await navigator.share({ title, text: textMsg, files });
        return true;
      } catch (e) {
    ...
    // ✅ One-by-one (reliable for mixed types OR when multi-file fails)
    // पहले IMAGES भेजें (एक साथ multi-file try), फिर PDFs/Videos individually
    try {
      // ✅ Group: images first (try multi), then PDFs (one-by-one), then videos (one-by-one)
      const imageDocs = docs.filter(d => d.fileType !== 'pdf' && d.fileType !== 'video');
      const pdfDocs   = docs.filter(d => d.fileType === 'pdf');
      const videoDocs = docs.filter(d => d.fileType === 'video');

      const imageFiles = imageDocs.map(d => {
        const ext = 'jpg', mime = 'image/jpeg';
        return dataURLtoFile(d.fileData, `${d.customerName}_${d.docTypeLabel}.${ext}`, mime);
      }).filter(Boolean);

      // Images: try multi-file (works in WhatsApp for images only)
      if (imageFiles.length > 0) {
        if (imageFiles.length === 1 || !navigator.canShare?.({ files: imageFiles })) {
          // Single image OR can't share multi
          for (let i = 0; i < imageFiles.length; i++) {
            try {
              await navigator.share({
                title: imageFiles[i].name,
                text: i === 0 ? textMsg : `📷 ${imageDocs[i]?.docTypeLabel || ''} (${i+1}/${imageFiles.length})`,
                files: [imageFiles[i]],
              });
          ...
          // Multi-image share (works for images only on most Android)
          try {
            await navigator.share({ title, text: textMsg, files: imageFiles });
          } catch (e) {
            if (e.name === 'AbortError') return false;
            // Multi failed — try one-by-one for images too
            for (let i = 0; i < imageFiles.length; i++) {
              try {
                await navigator.share({
                  title: imageFiles[i].name,
                ...
                await new Promise(r => setTimeout(r, 1200));
              } catch (e2) { if (e2.name === 'AbortError') return false; }
            }
          }
        }
      }

      // ✅ PDFs: हमेशा one-by-one (WhatsApp PDF अलग-अलग ही accept करता है)
      for (let i = 0; i < pdfDocs.length; i++) {
        const d = pdfDocs[i];
        const pdfFile = dataURLtoFile(d.fileData, `${d.customerName}_${d.docTypeLabel}.pdf`, 'application/pdf');
        if (!pdfFile) continue;
        await new Promise(r => setTimeout(r, 1500));
        try {
          await navigator.share({
            title: pdfFile.name,
            text: imageFiles.length === 0 && i === 0 ? textMsg : `📄 ${d.docTypeLabel}`,
            files: [pdfFile],
          });
        } catch (e) { if (e.name === 'AbortError') return false; }
      }

      // ✅ Videos: one-by-one (large files, individual better)
  ...
  // Desktop fallback: WhatsApp link + open files in tabs
