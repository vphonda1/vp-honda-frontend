// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-RESOLUTION-LADDER 🆕🆕🆕
// ✅ हर file पक्का ≤200KB — quality के बाद resolution भी घटती है + per-page files
// ═══════════════════════════════════════════════════════════════════════════
...
// ── मौजूदा PDF को छोटा करें ──────────────────────────────────────────────────
// पहले पूरी PDF एक ही file में 200KB के अंदर लाने की कोशिश।
// ज़्यादा pages हों और quality बहुत गिरने लगे — तो हर page की अलग file,
// जिससे हर file को पूरा 200KB का budget मिले और text साफ रहे।
async function compressExistingPdf(bytes, d, onProgress) {
  ...
  try {
    ...
    // ── कोशिश 1: सब pages एक ही PDF में ──
    if (pages.length <= 6) {
      const out = await PDFDocument.create();
      const per = Math.floor((LIMIT * 0.88) / pages.length);
      ...
      if (outBytes.length <= LIMIT) {
        return {
          files: [new File([outBytes], safeName(d, '.pdf'), { type: 'application/pdf' })],
          note: `${via} से ${origKB}KB → ${Math.round(outBytes.length/1024)}KB`,
        };
      }
      errors.push(`एक file में ${Math.round(outBytes.length/1024)}KB`);
    }

    // ── कोशिश 2: हर page की अपनी file — हर एक को पूरा 200KB budget ──
    const files = [];
    for (let i = 0; i < pages.length; i++) {
      onProgress?.(i + 1, pages.length);
      const one = await PDFDocument.create();
      const enc = await encodeBlobToPdfPage(one, pages[i], Math.floor(LIMIT * 0.85));
      const b = await one.save({ useObjectStreams: true });
      console.log(`[split p${i+1}] ${enc?.tag} -> ${Math.round(b.length/1024)}KB`);
      files.push(new File([b], safeName(d, `_p${i + 1}.pdf`), { type: 'application/pdf' }));
    }
    const biggest = Math.max(...files.map(f => f.size));
    if (files.length && biggest <= LIMIT) {
      return {
        files,
        note: `${via} · ${pages.length} pages अलग-अलग files में (हर एक ≤${Math.round(biggest/1024)}KB)`,
      };
    }
    errors.push(`page-wise में भी ${Math.round(biggest/1024)}KB`);
  } catch (e) {
    console.warn('[compressExistingPdf]', e?.message);
    errors.push(e?.message || 'fail');
  }

  // आखिरी सहारा — pdf-lib resave
  try {
    const { PDFDocument } = await import('pdf-lib');
    ...
    if (b.length < bytes.length) {
      return {
        files: [new File([b], safeName(d, '.pdf'), { type: 'application/pdf' })],
        note: b.length <= LIMIT ? `resave से ${origKB}KB → ${Math.round(b.length/1024)}KB`
                                : `⚠️ ${d.docTypeLabel} ${Math.round(b.length/1024)}KB — ${errors.join(' · ')}`,
      };
    }
  } catch (e) { errors.push(`resave: ${e?.message}`); }
