...
// ── Share files (RELIABLE: One-by-one + auto download fallback) ──────────────
async function shareFilesAuto(docs, textMsg, title) {
  // Convert all docs to File objects
  const files = docs.map(d => {
    ...
    return { file: dataURLtoFile(d.fileData, `${d.customerName}_${d.docTypeLabel}.${ext}`, mime), doc: d };
  }).filter(x => x.file);
  ...
  // ✅ NEW STRATEGY: Mobile WhatsApp multi-file drop करता है — हर file separately भेजते हैं
  if (navigator.share && navigator.canShare) {
    // Ask: auto share या download
    const choice = confirm(
      `📤 ${files.length} files send करनी हैं?\n\n` +
      `✅ OK — एक-एक करके share करें (हर file के लिए WhatsApp चुनना होगा)\n\n` +
      `❌ Cancel — सब files download करें + WhatsApp text भेजें (खुद attach करना)`
    );

    if (choice) {
      // Try multi-file first (works on some Android)
      if (navigator.canShare({ files: files.map(f => f.file) })) {
        try {
          await navigator.share({ title, text: textMsg, files: files.map(f => f.file) });
          // Sometimes multi-file appears to succeed but only text goes
          // Ask user to verify
          await new Promise(r => setTimeout(r, 2000));
          const allWorked = confirm(`✅ Files सही से गई?\n\nOK = हाँ सब गई\nCancel = एक-एक करके फिर भेजो`);
      ...
      // One-by-one (most reliable for WhatsApp)
      for (let i = 0; i < files.length; i++) {
        const { file, doc } = files[i];
        const caption = i === 0
          ? textMsg
          : `📎 *${doc.docTypeLabel}* (${i+1}/${files.length})\n${doc.customerName}`;
        try {
          await navigator.share({ title: file.name, text: caption, files: [file] });
          await new Promise(r => setTimeout(r, 1500));
        } catch (e) {
          if (e.name === 'AbortError') {
            const stop = confirm(`Share cancel हुआ। ${files.length - i - 1} files बाकी हैं।\n\nOK = बाकी skip करें\nCancel = जारी रखें`);
  ...
  // ✅ Download fallback (no Web Share API OR user chose download)
  showInAppToast('⬇️', `${files.length} files download हो रही हैं...`, 'info');
  ...
  const phone = prompt('📱 WhatsApp number डालें (10 digit) — text भेजने के लिए:');
  if (phone) {
    const finalMsg = textMsg + '\n\n📁 Documents download हो गए — WhatsApp में Gallery से attach करें';
    sendWhatsApp(phone, finalMsg);
  }
  return true;
}

// Old fallback (not used now) — kept for reference
async function shareFilesAutoOld(docs, textMsg, title) {
  // Convert all docs to File objects
  const files = docs.map(d => {
    if (!d.fileData) return null;
    const ext  = d.fileType === 'pdf' ? 'pdf' : d.fileType === 'video' ? 'mp4' : 'jpg';
  ...
  if (navigator.share) {
    try {
      ...
      await navigator.share({ title: `${title} - ${files[0].name}`, text: textMsg, files: [files[0]] });
      for (let i = 1; i < files.length; i++) {
        await new Promise(r => setTimeout(r, 1500));
        try {
          await navigator.share({ title: `${title} - ${files[i].name}`, text: `📎 File ${i+1}/${files.length}`, files: [files[i]] });
        } catch (e) {
          if (e.name === 'AbortError') return false;
        }
      }
