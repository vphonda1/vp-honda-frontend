// DocumentVault.jsx — VP Honda Document Vault — VERSION v2026-05-29-IndividualPDF
// 📌 IMPORTANT: यह version हर doc को अलग PDF बनाता है (कोई merge नहीं)
// 📌 अगर toast में "अलग-अलग PDFs बना रहे हैं" दिखे = NEW code चल रहा है
// 📌 अगर "एक PDF में merge हो रहे हैं" दिखे = OLD code, deployment problem
...
async function shareFilesAuto(docs, textMsg, title) {
  ...
  showInAppToast('📤', `${files.length} PDFs share कर रहे हैं: ${fileLabels.join(', ')}`, 'success');

  // ✅ STEP 2: सब PDFs एक साथ share — जैसे multi-image पहले काम कर रहा था
  if (navigator.share) {
    if (navigator.canShare && navigator.canShare({ files })) {
      try {
        ...
        console.warn('Multi-PDF share failed:', e.message);
      }
    }

    // अगर multi-PDF accept नहीं हुआ → text पहले अलग, फिर PDFs एक-एक करके
    showInAppToast('⚠️', 'Multi-share fail — पहले text, फिर PDFs भेज रहे हैं', 'warning');

    // ✅ STEP A: पहले सिर्फ text भेजें — WhatsApp PDF के साथ caption drop करता है
    try {
      await navigator.share({ title, text: textMsg });
      await new Promise(r => setTimeout(r, 1500));
    } catch (e) {
      if (e.name === 'AbortError') return false;
    }

    // ✅ STEP B: फिर हर PDF अलग-अलग भेजें
    for (let i = 0; i < files.length; i++) {
      const cap = `📄 ${fileLabels[i]} (${i+1}/${files.length})`;
      try {
