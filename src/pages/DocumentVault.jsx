...
export default function DocumentVault() {
  ...
  // ⚠️ FIX: यह dropdown खाली दिखने लगा था क्योंकि `/api/customers` से कुछ आ ही
  // नहीं रहा था (MongoDB का customers collection खाली हो चुका था). Backend अब
  // servicedatas + invoices से भी नाम भेजता है. यहाँ एक और सुरक्षा जोड़ी है —
  // अगर फिर भी list खाली मिले तो Vault में पहले से पड़े documents से ही
  // ग्राहक बना लिए जाते हैं, ताकि dropdown कभी बिल्कुल ख़ाली न रहे.
  const customerPool = useMemo(() => {
    if (customers.length) return customers;
    const seen = new Map();
    docs.forEach(d => {
      const key = (d.customerPhone || '').replace(/\D/g, '').slice(-10) || (d.customerName || '').toLowerCase();
      if (!key || seen.has(key)) return;
      seen.set(key, {
        customerName: d.customerName || '', name: d.customerName || '',
        phone: d.customerPhone || '', aadhar: d.aadharNo || '',
        vehicleModel: d.vehicleModel || '', chassisNo: d.chassisNo || '',
        nomineeName: d.nomineeName || '', hypothecation: d.hypothecation || '',
      });
    });
    return [...seen.values()];
  }, [customers, docs]);

  // ⚠️ पहले कम-से-कम 2 अक्षर चाहिए थे. अब 1 अक्षर से ही सूची खुल जाती है,
  // और खाना खाली हो पर cursor अंदर हो तो पहले 8 ग्राहक दिख जाते हैं.
  const filteredCustomers = (() => {
    const q = custSearch.trim().toLowerCase();
    if (!q) return customerPool.slice(0, 8);
    const qd = q.replace(/\D/g, '');
    return customerPool.filter(c =>
      (c.customerName || c.name || '').toLowerCase().includes(q) ||
      (qd && (c.mobileNo || c.phone || '').replace(/\D/g, '').includes(qd)) ||
      (qd && (c.aadhar || c.aadharNo || '').replace(/\D/g, '').includes(qd))
    ).slice(0, 8);
  })();
  ...
  const selectCustomer = (c) => {
    ...
    setForm({ ...form,
      ...
      // ⚠️ ये दोनों पहले भरे ही नहीं जाते थे — हर बार हाथ से लिखने पड़ते थे
      nomineeName:   c.nomineeName || form.nomineeName || '',
      hypothecation: c.hypothecation || form.hypothecation || '',
    });
  ...
  const handleCustNameChange = (value) => {
    ...
    setShowDropdown(true);   // पहले 2 अक्षर के बाद ही खुलता था
  };
            ...
            <div style={{ display:'grid', gap:10 }}>
              ...
              <div ref={dropdownRef} style={{ position:'relative' }}>
                ...
                <input value={custSearch} onChange={e => handleCustNameChange(e.target.value)}
                  ...
                  placeholder={customerPool.length ? `नाम लिखें — ${customerPool.length} ग्राहक में से चुनें` : 'नाम — auto-fill होगा'}
