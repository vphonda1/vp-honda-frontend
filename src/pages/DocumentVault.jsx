...
export default function DocumentVault() {
  ...
  const uploadToCloudinary = async (dataUrl, fileName) => {
    ...
    return data.secure_url; // ✅ सिर्फ URL return करेगा (MongoDB में URL save होगा — base64 नहीं)
  };
  ...
  const saveDoc = async (stayOpen = false) => {
    ...
    try {
      ...
      try {
        fileUrl = await uploadToCloudinary(fileData.dataUrl, fileData.fileName);
      } catch (cloudErr) {
        console.warn('Cloudinary failed, falling back to MongoDB:', cloudErr.message);
        // Fallback: base64 MongoDB में (old behaviour)
