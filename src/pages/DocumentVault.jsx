...
async function shareFilesAuto(docs, textMsg, title) {
  ...
  // ✅ STEP 2: ALWAYS text पहले अलग भेजें, फिर PDFs (multi-PDF reliable नहीं है)
  if (navigator.share) {
    // ── STEP A: सिर्फ TEXT message भेजें ──
    showInAppToast('📝 STEP 1/2', '✨ पहले TEXT message भेजें — contact select करें', 'info');
    try {
      await navigator.share({ title, text: textMsg });
      await new Promise(r => setTimeout(r, 1500));
    } catch (e) {
      if (e.name === 'AbortError') {
        showInAppToast('⚠️', 'Text cancel हुआ - PDFs भी skip', 'warning');
        return false;
      }
    }

    // ── STEP B: हर PDF अलग-अलग भेजें ──
    showInAppToast('📄 STEP 2/2', `अब ${files.length} PDFs एक-एक करके भेजें`, 'success');
    ...
    showInAppToast('✅', `सब भेज दिए: 1 text + ${files.length} PDFs`, 'success');
    return true;
  }
