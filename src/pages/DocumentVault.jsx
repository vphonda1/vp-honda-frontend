...
async function shareFilesAuto(docs, textMsg, title) {
  ...
  if (navigator.share) {
    ...
    // अगर multi-PDF accept नहीं हुआ → one-by-one fallback
    showInAppToast('⚠️', 'Multi-share fail — एक-एक करके भेज रहे हैं', 'warning');
    for (let i = 0; i < files.length; i++) {
      const text = i === 0 ? textMsg : `📄 ${fileLabels[i]} (${i+1}/${files.length})`;
      try {
        await navigator.share({ title: files[i].name, text, files: [files[i]] });
