// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-ONETAP-SHARE 🆕🆕🆕
// ✅ इस version में: upload memory fix + Cloudinary URL fix + 2-step one-tap share
// ═══════════════════════════════════════════════════════════════════════════
...
// ✅ SHARE FIX: PDF में डालने से पहले image को smart resize (1600px) करें
//    → WhatsApp share size कम, pixel quality फिर भी sharp (A4 पर ~220 DPI)
async function shrinkForPdf(bytes, mime, maxSide = 1600, quality = 0.85) {
  try {
    const blob = new Blob([bytes], { type: mime || 'image/jpeg' });
    const bitmap = await createImageBitmap(blob);
    let { width, height } = bitmap;
    const big = Math.max(width, height);
    if (big <= maxSide) { bitmap.close(); return null; } // पहले से छोटी — जैसी है वैसी रहने दें
    ...
    console.warn('[shrinkForPdf]', e?.message);
    return null;
  }
}
...
async function imageToPDF(d) {
  ...
  // ✅ Cloudinary URL OR base64 — दोनों support
  let { bytes: imgBytes, mime } = await getDocBytes(d);

  // ✅ बड़ी photo → resize (share fail होने का सबसे बड़ा कारण size था)
  const small = await shrinkForPdf(imgBytes, mime);
  if (small) { imgBytes = small.bytes; mime = small.mime; }

  let image;
  try { image = (mime || '').includes('png') ? await pdf.embedPng(imgBytes) : await pdf.embedJpg(imgBytes); }
  catch {
    // mime गलत हो तो दोनों तरीके try करें
  ...
  for (const d of valid) {
    try {
      if (d.fileType === 'pdf') {
        ...
        files.push(bytesToFile(bytes, safeName(d, '.pdf'), 'application/pdf'));
...
export default function DocumentVault() {
  ...
  const uploadToCloudinary = async (source, fileName) => {
  ...
  const saveDoc = async (stayOpen = false) => {
    ...
    try {
      ...
      try {
        ...
        fileUrl = await uploadToCloudinary(fileData.blob || fileData.dataUrl, fileData.fileName);
      } catch (cloudErr) {
            ...
            <div style={{ display:'grid', gap:6 }}>
              {sharePack.files.map((f, i) => (
                <button key={i} onClick={() => doShareOne(i)}
                  ...
                  <span style={{ fontSize:10, color:'#94a3b8', flexShrink:0 }}>{Math.round(f.size/1024)} KB</span>
