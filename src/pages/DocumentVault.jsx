// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-08-15-200KB-CAP 🆕🆕🆕
// ✅ upload memory fix + Cloudinary URL fix + one-tap share + हर PDF ≤ 200KB
// ═══════════════════════════════════════════════════════════════════════════
...
// ── Image resize (quality-safe) ───────────────────────────────────────────────
async function resizeImageBytes(bytes, mime, maxSide, quality) {
  ...
  ctx.imageSmoothingQuality = 'high';   // ✅ pixel फटेगा नहीं — smooth downscale
  ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, width, height); // transparent PNG के लिए सफेद background
  ctx.drawImage(bitmap, 0, 0, width, height);
  bitmap.close();
  const outBlob = await new Promise(res => canvas.toBlob(b => res(b), 'image/jpeg', quality));
  canvas.width = 0; canvas.height = 0;
  if (!outBlob) throw new Error('resize fail');
  const buf = await outBlob.arrayBuffer();
  return { bytes: new Uint8Array(buf), mime: 'image/jpeg' };
}

// एक image से A4 PDF बनाएं
async function makeImagePdf(imgBytes, mime, label) {
  const { PDFDocument, StandardFonts, rgb } = await import('pdf-lib');
  const pdf = await PDFDocument.create();
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  let image;
  try { image = (mime || '').includes('png') ? await pdf.embedPng(imgBytes) : await pdf.embedJpg(imgBytes); }
  catch {
    try { image = await pdf.embedJpg(imgBytes); }
    catch { try { image = await pdf.embedPng(imgBytes); } catch { throw new Error('Image format support नहीं'); } }
  }
...
// ✅ Quality ladder — पहले सबसे अच्छी quality, 200KB से बड़ी हो तो अगला step
//    (हर step पर सिर्फ थोड़ा कम — document readable ही रहेगा)
const QUALITY_STEPS = [
  { maxSide: 1600, quality: 0.82 },
  { maxSide: 1500, quality: 0.72 },
  { maxSide: 1400, quality: 0.62 },
  { maxSide: 1250, quality: 0.55 },
  { maxSide: 1100, quality: 0.48 },
  { maxSide: 1000, quality: 0.40 },
];
...
async function imageToPDF(d) {
  ...
  for (const step of QUALITY_STEPS) {
    try {
      const r   = await resizeImageBytes(rawBytes, rawMime, step.maxSide, step.quality);
      const pdf = await makeImagePdf(r.bytes, r.mime, d.docTypeLabel);
      if (!best || pdf.length < best.length) best = pdf;
      console.log(`[PDF ${d.docTypeLabel}] ${step.maxSide}px q${step.quality} → ${Math.round(pdf.length/1024)}KB`);
      if (pdf.length <= MAX_PDF_KB * 1024) { best = pdf; break; }
    } catch (e) { console.warn('[imageToPDF step]', e?.message); }
  }
  if (!best) {
    // सारे resize fail — original से बनाने की आखिरी कोशिश
    best = await makeImagePdf(rawBytes, rawMime, d.docTypeLabel);
  }
  return new File([best], safeName(d, '.pdf'), { type: 'application/pdf' });
}

// ── मौजूदा PDF को छोटा करें (pdf.js से हर page को image बनाकर) ───────────────
//    pdfjs-dist न हो तो original PDF ही भेज देंगे (कुछ टूटेगा नहीं)
async function compressExistingPdf(bytes, d) {
  ...
  try {
    ...
    const attempt = async (scale, quality) => {
      ...
      for (let p = 1; p <= pageCount; p++) {
        ...
        const ratio = Math.min(575 / img.width, 822 / img.height);
    ...
    // ✅ multi-page PDF में per-page budget कम रखें
    const steps = [[1.7, 0.75], [1.5, 0.62], [1.3, 0.52], [1.1, 0.42], [0.95, 0.35]];
    let best = null;
    for (const [scale, q] of steps) {
      try {
        const outBytes = await attempt(scale, q);
        if (!best || outBytes.length < best.length) best = outBytes;
        console.log(`[PDF-shrink ${d.docTypeLabel}] scale${scale} q${q} → ${Math.round(outBytes.length/1024)}KB`);
        if (outBytes.length <= MAX_PDF_KB * 1024) { best = outBytes; break; }
      } catch (e) { console.warn('[pdf shrink step]', e?.message); }
    }
    if (best && best.length < bytes.length) {
      return new File([best], safeName(d, '.pdf'), { type: 'application/pdf' });
    }
  } catch (e) {
    console.warn('[compressExistingPdf] pdfjs-dist नहीं मिली या fail:', e?.message);
  }
...
async function buildShareFiles(docs) {
  ...
  for (const d of valid) {
    try {
      if (d.fileType === 'pdf') {
        ...
        files.push(await compressExistingPdf(bytes, d));   // ✅ 200KB से बड़ी PDF छोटी होगी
...
export default function DocumentVault() {
  ...
  const prepareShare = async (validDocs, textMsg, title) => {
    ...
    try {
      const { files, fileLabels, failReasons, totalKB } = await buildShareFiles(validDocs);
      if (files.length === 0) {
      ...
      {preparing && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:210 }}>
          <div style={{ background:'#0f172a', border:'1px solid #334155', borderRadius:14, padding:'22px 28px', textAlign:'center' }}>
            ...
            <p style={{ color:'#94a3b8', fontSize:10, margin:'4px 0 0' }}>photos compress + PDF बन रही हैं</p>
          </div>
        </div>
      )}
