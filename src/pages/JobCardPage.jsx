          ...
          <Card className="shadow-xl">
            ...
            <CardContent className="pt-6 space-y-4 max-h-screen overflow-y-auto">
              ...
              <div className="border-3 border-blue-500 p-4 rounded bg-blue-50">
                ...
                {/* Search box — focus पर सब show, type पर filter */}
                <div style={{ position:'relative' }}>
                  <input
                    type="text"
                    value={custSearch}
                    onChange={e => { setCustSearch(e.target.value); setFormData(f=>({...f, customerId:''})); }}
                    onFocus={() => setShowCustList(true)}
                    onBlur={() => setTimeout(()=>setShowCustList(false), 200)}
                    placeholder="🔍 नाम, phone, reg no से search करें... (click करें या type करें)"
                    className="w-full border-2 border-blue-400 p-2 font-bold text-base rounded bg-white"
                    autoComplete="off"
                  />

                  {/* Dropdown list */}
                  {showCustList && (
                    <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:100, maxHeight:220, overflowY:'auto', background:'#fff', border:'2px solid #93c5fd', borderRadius:8, boxShadow:'0 4px 20px rgba(0,0,0,0.15)' }}>
                      {(() => {
                        const allC = customerType === 'existing' ? customers : newCustomers;
                        const q = custSearch.toLowerCase().trim();
                        const filtered = q
                          ? allC.filter(c =>
                              (c.customerName||c.name||'').toLowerCase().includes(q) ||
                              (c.mobileNo||c.phone||'').includes(q) ||
                              (c.regNo||c.registrationNo||'').toLowerCase().includes(q)
                            )
                          : allC.slice(0, 30); // empty search → first 30
                        if (filtered.length === 0) return <div style={{padding:12,color:'#94a3b8',fontSize:13}}>कोई customer नहीं मिला</div>;
                        return filtered.map(c => (
                          <div key={c._id}
                            onMouseDown={() => { setFormData(f=>({...f, customerId:c._id})); setCustSearch(c.customerName||c.name||''); setShowCustList(false); }}
                            style={{ padding:'8px 12px', borderBottom:'1px solid #e2e8f0', cursor:'pointer', fontSize:13 }}
                            onMouseEnter={e=>e.currentTarget.style.background='#eff6ff'}
                            onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
                            <div style={{fontWeight:700}}>{c.customerName||c.name||'Unknown'}</div>
                            <div style={{color:'#64748b',fontSize:11}}>📞 {c.mobileNo||c.phone||'—'} · 🏍️ {c.vehicleModel||'—'} · {c.regNo||''}</div>
                          </div>
                        ));
                      })()}
                      {!custSearch && <div style={{padding:'6px 12px',color:'#94a3b8',fontSize:11,borderTop:'1px solid #e2e8f0'}}>पूरी list देखने के लिए type करें</div>}
                    </div>
                  )}
                </div>
                ...
                {formData.customerId && (
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:6 }}>
                    <span style={{ color:'#16a34a', fontSize:12, fontWeight:700 }}>✅ Selected</span>
                    <button type="button" onClick={() => { setFormData(f=>({...f, customerId:''})); setCustSearch(''); }}
                      style={{ fontSize:11, color:'#ef4444', background:'none', border:'none', cursor:'pointer', textDecoration:'underline' }}>Change</button>
                  </div>
                )}
              </div>
              ...
              <div className="border-2 border-indigo-500 p-4 rounded bg-indigo-50">
                ...
                <div className="mb-4" style={{ position:'relative' }}>
                  <input
                    ...
                    placeholder="🔍 Part description या Part No से search... (click करें)"
                    className="w-full border-2 border-indigo-400 p-2 font-bold text-sm rounded bg-white"
                    autoComplete="off"
                  />
                  {showPartList && (
                    <div style={{ position:'absolute', top:'100%', left:0, right:0, zIndex:100, maxHeight:220, overflowY:'auto', background:'#fff', border:'2px solid #a5b4fc', borderRadius:8, boxShadow:'0 4px 20px rgba(0,0,0,0.15)' }}>
                      {(() => {
                        ...
                        const filtered = q
                          ...
                          : parts.slice(0, 30);
                        if (filtered.length === 0) return <div style={{padding:12,color:'#94a3b8',fontSize:13}}>कोई part नहीं मिला</div>;
                        return filtered.map(p => {
                          const already = selectedParts.find(sp => sp._id === p._id);
                          return (
                            <div key={p._id}
                              onMouseDown={() => { if (!already) { addPart(p._id); setPartSearch(''); setShowPartList(false); } }}
                              style={{ padding:'8px 12px', borderBottom:'1px solid #e2e8f0', cursor: already?'default':'pointer', background: already?'#f1f5f9':'#fff', fontSize:13, display:'flex', justifyContent:'space-between', alignItems:'center' }}
                              onMouseEnter={e=>{ if (!already) e.currentTarget.style.background='#eef2ff'; }}
                              onMouseLeave={e=>{ e.currentTarget.style.background=already?'#f1f5f9':'#fff'; }}>
                              <div>
                                <div style={{fontWeight:700}}>{p.description}</div>
                                <div style={{color:'#64748b',fontSize:11}}>Part No: {p.partNo||'—'} · HSN: {p.hsnCode||'—'} · ₹{p.sellingPrice||p.mrp||'—'}</div>
                              </div>
                              {already
                                ? <span style={{color:'#16a34a',fontSize:11,fontWeight:700,whiteSpace:'nowrap'}}>✅ Added</span>
                                : <span style={{color:'#4f46e5',fontSize:11,fontWeight:700,whiteSpace:'nowrap'}}>+ Add</span>}
                            </div>
                          );
                        });
                      })()}
                      {!partSearch && <div style={{padding:'6px 12px',color:'#94a3b8',fontSize:11,borderTop:'1px solid #e2e8f0'}}>पूरी list देखने के लिए type करें</div>}
                    </div>
                  )}
                </div>
