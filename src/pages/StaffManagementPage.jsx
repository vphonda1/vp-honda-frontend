...
// ════════════════════════════════════════════════════════════════════════════
// रक़म को शब्दों में — हर पेशेवर salary slip में यह होता है
// ════════════════════════════════════════════════════════════════════════════
// (TeamLease की slip में "Thirteen Thousand Five Hundred Fifty Three rupees"
//  लिखा है — VP Honda की slip में यह नहीं था. बैंक और आयकर, दोनों जगह
//  शब्दों में रक़म माँगी जाती है.)
...
const StaffFormFields = ({ data, setData, showBank = true }) => (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
      ...
      {/* ⚠️ पहले सिर्फ़ "पद" था और slip में Designation व Department दोनों जगह
          वही छपता था. अब विभाग अलग से चुना जा सकता है. */}
      <div><label className="text-slate-400 text-xs mb-1 block">विभाग (Department)</label>
        <select value={data.department || ''} onChange={e => setData({ ...data, department: e.target.value })} className="w-full px-3 py-2 bg-slate-700 text-white border border-slate-600 rounded-md text-sm">
          <option value="">— चुनें —</option>
          <option value="Sales">Sales — गाड़ी बिक्री</option>
          <option value="Service">Service — वर्कशॉप</option>
          <option value="Spare Parts">Spare Parts — पार्ट्स</option>
          <option value="Accounts">Accounts — हिसाब</option>
          <option value="Front Office">Front Office — रिसेप्शन</option>
          <option value="Administration">Administration — प्रबंधन</option>
        </select></div>
      ...
      {showBank && <>
        ...
        <div><label className="text-slate-400 text-xs mb-1 block">📱 UPI ID</label>
          <Input value={data.upiId || ''} onChange={e => setData({ ...data, upiId: e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="name@upi" /></div>

        {/* ✅ Emergency Contact */}
        <div className="col-span-2 mt-2 mb-1"><p className="text-amber-400 text-xs font-bold">🆘 Emergency Contact</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">Emergency Contact Name</label>
          <Input value={data.emergencyName || ''} onChange={e => setData({ ...data, emergencyName: e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="Name" /></div>
        <div><label className="text-slate-400 text-xs mb-1 block">Emergency Phone</label>
          <Input value={data.emergencyPhone || ''} onChange={e => setData({ ...data, emergencyPhone: e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="Mobile" /></div>
        <div><label className="text-slate-400 text-xs mb-1 block">Relation</label>
          <Input value={data.emergencyRelation || ''} onChange={e => setData({ ...data, emergencyRelation: e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="Father/Wife/Brother..." /></div>

        {/* ✅ Salary Structure */}
        <div className="col-span-2 mt-2 mb-1"><p className="text-green-400 text-xs font-bold">💰 Salary Structure (Monthly ₹{(data.monthlySalary||0).toLocaleString('en-IN')} breakdown)</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">HRA % (Basic का)</label>
          <Input type="number" value={data.hraPercent ?? 20} onChange={e => setData({ ...data, hraPercent: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="20"/>
          <p className="text-slate-500 text-xs mt-0.5">= ₹{Math.round((data.monthlySalary||0)*(data.hraPercent??20)/100).toLocaleString('en-IN')}/mo</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">DA % (Basic का)</label>
          <Input type="number" value={data.daPercent ?? 10} onChange={e => setData({ ...data, daPercent: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="10"/>
          <p className="text-slate-500 text-xs mt-0.5">= ₹{Math.round((data.monthlySalary||0)*(data.daPercent??10)/100).toLocaleString('en-IN')}/mo</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">PF % (employer)</label>
          <Input type="number" value={data.pfPercent ?? 12} onChange={e => setData({ ...data, pfPercent: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="12"/>
          <p className="text-slate-500 text-xs mt-0.5">= ₹{Math.round((data.monthlySalary||0)*(data.pfPercent??12)/100).toLocaleString('en-IN')}/mo</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">ESI % (applicable)</label>
          <Input type="number" value={data.esiPercent ?? 0.75} step="0.25" onChange={e => setData({ ...data, esiPercent: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="0.75"/>
          <p className="text-slate-500 text-xs mt-0.5">= ₹{Math.round((data.monthlySalary||0)*(data.esiPercent??0.75)/100).toLocaleString('en-IN')}/mo</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">TDS % (if applicable)</label>
          <Input type="number" value={data.tdsPercent ?? 0} step="0.5" onChange={e => setData({ ...data, tdsPercent: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="0"/>
          <p className="text-slate-500 text-xs mt-0.5">= ₹{Math.round((data.monthlySalary||0)*(data.tdsPercent??0)/100).toLocaleString('en-IN')}/mo</p></div>
        <div><label className="text-slate-400 text-xs mb-1 block">Other Allowance ₹/mo</label>
          <Input type="number" value={data.otherAllowance ?? 0} onChange={e => setData({ ...data, otherAllowance: +e.target.value })} className="bg-slate-700 text-white border-slate-600" placeholder="0"/>
          <p className="text-slate-500 text-xs mt-0.5">Fuel/Travel/Mobile allowance</p></div>
...
export default function StaffManagementPage() {
  ...
  const emptyStaff = {
    ...
    aadharNo: '', panNo: '', position: 'Mechanic', department: '',
    monthlySalary: 0, joinDate: new Date().toISOString().split('T')[0],
    bankAccount: '', ifscCode: '', bankBranch: '', bankName: '', upiId: '',
    emergencyName: '', emergencyPhone: '', emergencyRelation: '',
    hraPercent: 20, daPercent: 10, pfPercent: 12, esiPercent: 0.75, tdsPercent: 0, otherAllowance: 0,
    pin: '1234'
  };
  ...
  const [paySlipExtras, setPaySlipExtras] = useState({
    ...
    // ⭐ TeamLease की slip जैसी और मदें
    conveyance: 0, worksAllowance: 0, manualIncentive: 0,
    // ⭐ हाथ से भरी attendance — पिछले महीनों के लिए, जिनका record app में नहीं है
    manualPresent: '', manualLate: '',
  });

  useEffect(() => { loadData(); }, []);

  // ⭐ Auto-refresh salary data every 5 minutes (attendance edit interrupt से बचाने के लिए)
  useEffect(() => {
  ...
  // एक बार तुरंत चलती है. यही सबसे बड़ी बचत है — service सो पाती है.
    const stopPoll = visibleInterval(() => { loadSalaryData(); }, 600000); // 5 → 10 मिनट
    return stopPoll;
  }, [staffList]);
  ...
  const downloadPaySlipPDF = () => {
    ...
    // ⚠️ FIX — पिछले महीनों की attendance app में दर्ज ही नहीं है (attendance
    // feature बाद में शुरू हुआ). पहले सीधे `att.length` लिया जाता था, इसलिए
    // पुराने महीने की slip में "1 उपस्थित, 25 अनुपस्थित" जैसा गलत हिसाब आता था
    // और slip देने लायक ही नहीं रहती थी.
    //
    // अब: अगर उस महीने का कोई record नहीं मिला तो **पूरा महीना उपस्थित** माना
    // जाता है (यही सही मान्यता है — दर्ज न होने का मतलब गैरहाज़िर होना नहीं).
    // और नीचे form में हाथ से भी भरा जा सकता है, जो सबसे ऊपर रहता है.
    const autoPresent = att.length;
    const hasRecords  = autoPresent > 0;

    const mPresent = ex.manualPresent !== '' && ex.manualPresent !== undefined && ex.manualPresent !== null
      ? Math.max(0, Math.min(wDays, parseInt(ex.manualPresent, 10) || 0)) : null;
    const mLate = ex.manualLate !== '' && ex.manualLate !== undefined && ex.manualLate !== null
      ? Math.max(0, parseInt(ex.manualLate, 10) || 0) : null;

    const present = mPresent !== null ? mPresent : (hasRecords ? autoPresent : wDays);
    const absent  = Math.max(0, wDays - present);
    const late    = mLate !== null ? mLate
                  : att.filter(a => a.checkInTime && isLateTime(a.checkInTime)).length;

    // slip पर बताया जाए कि गिनती कहाँ से आई
    const attSource = mPresent !== null ? 'हाथ से भरी गई'
                    : hasRecords        ? 'App की attendance से'
                    : 'Record नहीं — पूरा महीना उपस्थित माना गया';
    ...
    // ⚠️ पहले सिर्फ़ Basic + DA + HRA + Other थे. TeamLease की slip में
    // Conveyance और Works Allowance अलग मदें हैं — वे भी जोड़ीं.
    const conveyance     = parseFloat(ex.conveyance || 0);
    const worksAllowance = parseFloat(ex.worksAllowance || 0);
    const totalEarnings  = base + parseFloat(ex.da||0) + parseFloat(ex.hra||0)
                         + parseFloat(ex.otherAllowance||0) + conveyance + worksAllowance;

    const incList = (incentiveData[staff.id] || []).filter(i => i.month === m && i.year === y);
    // ⚠️ Incentive अब तक सिर्फ़ Incentive/Bonus module से आता था — यानी वहाँ
    // entry न हो तो slip में incentive की कोई line ही नहीं आती थी.
    // अब payslip form में हाथ से भी भरा जा सकता है, दोनों जुड़ जाते हैं.
    const incentive = incList.filter(i => i.type === 'incentive').reduce((s, i) => s + i.amount, 0)
                    + parseFloat(ex.manualIncentive || 0);
    ...
    // ⚠️ पहले Designation और Department दोनों में `staff.position` ही छपता था,
    // इसलिए slip पर एक ही शब्द दो बार दिखता. अब department अलग से भरा जाता है;
    // न भरा हो तो पद से अंदाज़ा लगा लेते हैं — और वही शब्द दोबारा नहीं छपता.
    const DEPT_BY_ROLE = {
      'Mechanic':'Service', 'Helper':'Service',
      'Sales Executive':'Sales', 'Manager':'Administration',
      'Receptionist':'Front Office', 'Accountant':'Accounts',
    };
    const deptName = staff.department
      || DEPT_BY_ROLE[staff.position]
      || '—';
    // slip का अपना नंबर — बाद में मिलान के लिए (जैसे TeamLease का Emp No)
    const slipNo = `VPH/${String(y).slice(2)}${String(m).padStart(2,'0')}/${String(staff.id || '').slice(-4) || '0001'}`;
...
<div style="font-family:'Segoe UI',Arial,sans-serif;background:#f1f5f9;margin:0;padding:0;">
  ...
  <div style="margin:10px 18px 0;background:white;border-radius:10px;overflow:hidden;box-shadow:0 3px 12px rgba(0,0,0,0.07);">
    ...
    <div style="padding:10px 14px;">
      <table style="width:100%;font-size:11px;border-collapse:collapse;">
        ...
        <tr>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Employee ID</td>
          <td style="padding:3px 8px 3px 0;font-weight:600;">${slipNo}</td>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Department</td>
          <td style="padding:3px 0;">${deptName}</td>
        </tr>
        <tr>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Mobile / PAN</td>
          <td style="padding:3px 8px 3px 0;">${staff.phone || '—'} &nbsp;|&nbsp; ${staff.panNo || '—'}</td>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Date of Joining</td>
          <td style="padding:3px 0;">${staff.joinDate ? new Date(staff.joinDate).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' }) : '—'}</td>
        </tr>
        <tr>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Aadhar</td>
          <td style="padding:3px 8px 3px 0;">${staff.aadharNo || '—'}</td>
          <td style="padding:3px 8px 3px 0;color:#64748b;">Pay Period</td>
          <td style="padding:3px 0;font-weight:600;">${mName} &nbsp;(${daysInM} days)</td>
        </tr>
  ...
  <!-- ⚠️ Attendance का 6-box menu हटा दिया गया —
       कारण: पुराने महीनों का record app में नहीं है, इसलिए slip पर
       "1 उपस्थित, 25 अनुपस्थित" जैसा गलत हिसाब छपता था. गिनती अब सिर्फ़
       Deductions में इस्तेमाल होती है, slip पर नहीं दिखती. -->
    ...
    <div style="flex:1;background:white;border-radius:10px;overflow:hidden;box-shadow:0 3px 12px rgba(0,0,0,0.07);">
      ...
      <div style="padding:8px 10px;">
        <table style="width:100%;font-size:11px;border-collapse:collapse;">
          ...
          ${conveyance>0 ? `<tr><td style="padding:6px 5px;border-bottom:1px solid #f5f5f5;">Conveyance</td><td style="padding:6px 5px;text-align:right;border-bottom:1px solid #f5f5f5;">${fmt(conveyance)}</td></tr>` : ''}
          ${worksAllowance>0 ? `<tr><td style="padding:6px 5px;background:#f9fafb;">Works Allowance</td><td style="padding:6px 5px;background:#f9fafb;text-align:right;">${fmt(worksAllowance)}</td></tr>` : ''}
  ...
  <!-- ⚠️ पहले "Amount in Words" और signature साथ-साथ (बग़ल में) थे, इसलिए
       पन्ना एक तरफ़ भारी लगता था और नीचे ख़ाली जगह बच जाती थी.
       अब Amount in Words पूरी चौड़ाई में, और signature उसके नीचे — पन्ना बराबर. -->

  <!-- AMOUNT IN WORDS + SUMMARY (पूरी चौड़ाई) -->
  <div style="margin:6px 18px 0;background:white;border-radius:10px;padding:9px 14px;box-shadow:0 3px 12px rgba(0,0,0,0.07);border-left:4px solid #16a34a;">
    <table style="width:100%;">
      <tr>
        <td style="vertical-align:top;width:52%;padding-right:14px;">
          <div style="font-size:9px;font-weight:700;color:#15803d;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">Amount in Words</div>
          <div style="font-size:12px;font-weight:700;color:#0f172a;line-height:1.5;">${amountWords}</div>
        </td>
        <td style="vertical-align:top;width:48%;border-left:1px solid #f1f5f9;padding-left:14px;">
          <table style="width:100%;font-size:9px;color:#64748b;border-collapse:collapse;">
            <tr>
              <td style="padding:2px 6px 2px 0;">Working Days</td><td style="padding:2px 12px 2px 0;font-weight:700;color:#334155;">${wDays}</td>
              <td style="padding:2px 6px 2px 0;">Paid Days</td><td style="padding:2px 0;font-weight:700;color:#334155;">${present}</td>
            </tr>
            <tr>
              <td style="padding:2px 6px 2px 0;">LOP</td><td style="padding:2px 12px 2px 0;font-weight:700;color:#334155;">${absent}</td>
              <td style="padding:2px 6px 2px 0;">Per Day</td><td style="padding:2px 0;font-weight:700;color:#334155;">${fmt(perDay)}</td>
            </tr>
            <tr>
              <td style="padding:2px 6px 2px 0;">Pay Mode</td><td style="padding:2px 12px 2px 0;font-weight:700;color:#334155;">${staff.bankName ? 'Bank Transfer' : 'Cash'}</td>
              <td style="padding:2px 6px 2px 0;">Slip No.</td><td style="padding:2px 0;font-weight:700;color:#334155;">${slipNo}</td>
            </tr>
          </table>
          <div style="font-size:7.5px;color:#94a3b8;margin-top:5px;">Attendance: ${attSource}</div>
        </td>
      </tr>
    </table>
  </div>

  <!-- SIGNATURES (उसी के नीचे, पूरी चौड़ाई) -->
  <div style="margin:6px 18px 0;background:white;border-radius:10px;padding:10px 14px 8px;box-shadow:0 3px 12px rgba(0,0,0,0.07);">
    <table style="width:100%;">
      <tr>
        <td style="text-align:center;width:50%;vertical-align:bottom;">
          <div style="border-top:2px solid #e2e8f0;padding-top:5px;margin-top:26px;width:70%;margin-left:auto;margin-right:auto;">
            <div style="font-size:10.5px;color:#374151;font-weight:600;">${staff.name}</div>
            <div style="font-size:8.5px;color:#94a3b8;">Employee Signature</div>
          </div>
        </td>
        <td style="text-align:center;width:50%;vertical-align:bottom;">
          <div style="border-top:2px solid #e2e8f0;padding-top:5px;margin-top:26px;width:70%;margin-left:auto;margin-right:auto;">
            <div style="font-size:10.5px;color:#374151;font-weight:600;">Authorised Signatory</div>
  ...
  <div style="background:linear-gradient(90deg,#0f172a,#1e3a8a);color:rgba(255,255,255,0.6);padding:9px 18px;text-align:center;font-size:9px;">
    This is a computer-generated salary slip &nbsp;|&nbsp; VP Honda Dealership © ${y} &nbsp;|&nbsp; ${slipNo} &nbsp;|&nbsp; Generated: ${new Date().toLocaleString('en-IN')}
  </div>
</div>`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <div className="max-w-7xl mx-auto space-y-4">
            ...
            <div className="w-full max-w-md mx-4">
              ...
              <div className="text-center mb-8">
                <img
                  ...
                  width={96}
                  height={96}
                  className="h-24 w-24 mx-auto mb-3 object-contain drop-shadow-2xl"
                />
                  ...
                  {isExpanded && (
                    <CardContent className="pt-4 pb-4">
                      ...
                      <div className="bg-slate-700/50 rounded-lg p-3 mb-4 text-xs text-slate-300 grid grid-cols-2 md:grid-cols-4 gap-2">
                        ...
                        {staff.upiId && <div><span className="text-slate-500">UPI: </span><span className="text-green-400">{staff.upiId}</span></div>}
                      </div>

                      {/* ✅ Emergency Contact */}
                      {staff.emergencyName && (
                        <div className="bg-amber-900/20 border border-amber-700 rounded-lg p-3 mb-3 text-xs">
                          <p className="text-amber-400 font-bold mb-1">🆘 Emergency Contact</p>
                          <div className="text-slate-300">{staff.emergencyName} ({staff.emergencyRelation||'—'})</div>
                          <div className="text-amber-300 font-bold">{staff.emergencyPhone}</div>
                        </div>
                      )}

                      {/* ✅ Salary Structure */}
                      {staff.monthlySalary > 0 && (
                        <div className="bg-green-900/20 border border-green-700 rounded-lg p-3 mb-3 text-xs">
                          <p className="text-green-400 font-bold mb-2">💰 Salary Structure — ₹{(staff.monthlySalary||0).toLocaleString('en-IN')}/mo</p>
                          <div className="grid grid-cols-3 gap-1">
                            <div className="bg-slate-700 rounded p-1.5"><div className="text-slate-400">HRA</div><div className="text-white font-bold">₹{Math.round((staff.monthlySalary||0)*(staff.hraPercent||20)/100).toLocaleString('en-IN')}</div><div className="text-slate-500">{staff.hraPercent||20}%</div></div>
                            <div className="bg-slate-700 rounded p-1.5"><div className="text-slate-400">DA</div><div className="text-white font-bold">₹{Math.round((staff.monthlySalary||0)*(staff.daPercent||10)/100).toLocaleString('en-IN')}</div><div className="text-slate-500">{staff.daPercent||10}%</div></div>
                            <div className="bg-slate-700 rounded p-1.5"><div className="text-slate-400">Allowance</div><div className="text-white font-bold">₹{(staff.otherAllowance||0).toLocaleString('en-IN')}</div><div className="text-slate-500">fixed</div></div>
                            <div className="bg-red-900/30 rounded p-1.5"><div className="text-slate-400">PF</div><div className="text-red-300 font-bold">-₹{Math.round((staff.monthlySalary||0)*(staff.pfPercent||12)/100).toLocaleString('en-IN')}</div><div className="text-slate-500">{staff.pfPercent||12}%</div></div>
                            <div className="bg-red-900/30 rounded p-1.5"><div className="text-slate-400">ESI</div><div className="text-red-300 font-bold">-₹{Math.round((staff.monthlySalary||0)*(staff.esiPercent||0.75)/100).toLocaleString('en-IN')}</div><div className="text-slate-500">{staff.esiPercent||0.75}%</div></div>
                            <div className="bg-red-900/30 rounded p-1.5"><div className="text-slate-400">TDS</div><div className="text-red-300 font-bold">-₹{Math.round((staff.monthlySalary||0)*(staff.tdsPercent||0)/100).toLocaleString('en-IN')}</div><div className="text-slate-500">{staff.tdsPercent||0}%</div></div>
                          </div>
                          <div className="mt-2 flex justify-between font-bold text-sm">
                            <span className="text-slate-400">Net Take-Home:</span>
                            <span className="text-green-400">₹{Math.max(0, (staff.monthlySalary||0) + (staff.otherAllowance||0) - Math.round((staff.monthlySalary||0)*((staff.pfPercent||12)+(staff.esiPercent||0.75)+(staff.tdsPercent||0))/100)).toLocaleString('en-IN')}</span>
                          </div>
                        </div>
                      )}
            ...
            <CardContent className="pt-4 space-y-4">
              ...
              <div>
                ...
                {isAdmin ? (
                  <select
                    ...
                    onChange={e => setSelectedStaffPaySlip(staffList.find(s => String(s.id) === e.target.value))}
              ...
              {paySlipType === 'monthly' && (
                <div className="bg-slate-700 p-4 rounded-lg space-y-3">
                  ...
                  {/* ⭐ हाथ से attendance — पिछले महीनों के लिए */}
                  {isAdmin && (
                    <div className="bg-amber-900/25 border border-amber-700/40 rounded-lg p-3 space-y-2 mb-3">
                      <p className="text-amber-300 text-xs font-bold">📅 Attendance — हाथ से भरें</p>
                      <p className="text-slate-400 text-[11px] leading-relaxed">
                        जिन पुराने महीनों की attendance app में दर्ज नहीं है, उनके लिए यहाँ भर दें।
                        <b className="text-slate-300"> खाली छोड़ेंगे</b> तो app की attendance ली जाएगी;
                        वह भी न हो तो <b className="text-slate-300">पूरा महीना उपस्थित</b> माना जाएगा।
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-slate-400 text-xs">उपस्थित दिन</label>
                          <Input type="number" min="0"
                            value={paySlipExtras.manualPresent}
                            onChange={e => setPaySlipExtras({ ...paySlipExtras, manualPresent: e.target.value })}
                            placeholder="खाली = अपने आप"
                            className="bg-slate-700 text-white border-slate-600 h-8 text-sm"/>
                        </div>
                        <div>
                          <label className="text-slate-400 text-xs">लेट दिन</label>
                          <Input type="number" min="0"
                            value={paySlipExtras.manualLate}
                            onChange={e => setPaySlipExtras({ ...paySlipExtras, manualLate: e.target.value })}
                            placeholder="खाली = अपने आप"
                            className="bg-slate-700 text-white border-slate-600 h-8 text-sm"/>
                        </div>
                      </div>
                      {selectedStaffPaySlip && (
                        <p className="text-slate-500 text-[11px]">
                          {MONTHS[selectedMonthPaySlip - 1]} {selectedYearPaySlip} में{' '}
                          {new Date(selectedYearPaySlip, selectedMonthPaySlip, 0).getDate()
                            - countSundays(selectedYearPaySlip, selectedMonthPaySlip - 1)} working days हैं
                          (रविवार छोड़कर)
                        </p>
                      )}
                    </div>
                  ...
                  {isAdmin && (
                    <div className="bg-slate-600/50 rounded-lg p-3 space-y-2">
                      <p className="text-slate-300 text-xs font-bold">➕ Optional: Allowances &amp; Incentive (0 रहने पर PDF में नहीं दिखेगा)</p>
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div><label className="text-slate-400 text-xs">Conveyance (₹)</label>
                          <Input type="number" value={paySlipExtras.conveyance||''} onChange={e=>setPaySlipExtras({...paySlipExtras,conveyance:e.target.value})} placeholder="0" className="bg-slate-700 text-white border-slate-600 h-8 text-sm"/></div>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <div><label className="text-slate-400 text-xs">Works Allow. (₹)</label>
                          <Input type="number" value={paySlipExtras.worksAllowance||''} onChange={e=>setPaySlipExtras({...paySlipExtras,worksAllowance:e.target.value})} placeholder="0" className="bg-slate-700 text-white border-slate-600 h-8 text-sm"/></div>
                        ...
                        <div><label className="text-emerald-400 text-xs">⭐ Incentive (₹)</label>
                          <Input type="number" value={paySlipExtras.manualIncentive||''} onChange={e=>setPaySlipExtras({...paySlipExtras,manualIncentive:e.target.value})} placeholder="0" className="bg-slate-700 text-white border-emerald-700 h-8 text-sm"/></div>
                      </div>
                      <p className="text-slate-500 text-[10px]">
                        ⭐ Incentive यहाँ भरा हुआ + Incentive/Bonus module वाला — दोनों जुड़कर slip में आते हैं।
                      </p>
        ...
        {/* ══════════════════════════════════════════════════════════════════
            ⚡ NEW: Penalty, Incentive & Leaderboard Module
        ══════════════════════════════════════════════════════════════════ */}
        {isAdmin && <StaffModulesPanel staffList={staffList}/>}
...
// ════════════════════════════════════════════════════════════════════════════
// ⚡ StaffModulesPanel — Penalty, Incentive, Leaderboard
// ════════════════════════════════════════════════════════════════════════════
function StaffModulesPanel({ staffList }) {
  const [tab,       setTab]       = useState('penalty');
  const [penalties, setPenalties] = useState([]);
  const [incentives,setIncentives]= useState([]);
  const [loading,   setLoading]   = useState(false);
  const [pForm,     setPForm]     = useState({ staffName:'', category:'late_coming', amount:'', description:'', date:new Date().toISOString().split('T')[0] });
  const [iForm,     setIForm]     = useState({ staffName:'', category:'retail', amount:'', units:'1', perUnit:'', description:'', date:new Date().toISOString().split('T')[0] });
  const [saving,    setSaving]    = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [pr, ir] = await Promise.all([
        fetch(`${API}/api/staff-modules/penalties?month=${month}&year=${year}`).then(r=>r.ok?r.json():[]).catch(()=>[]),
        fetch(`${API}/api/staff-modules/incentives?month=${month}&year=${year}`).then(r=>r.ok?r.json():[]).catch(()=>[]),
      ]);
      setPenalties(Array.isArray(pr)?pr:[]);
      setIncentives(Array.isArray(ir)?ir:[]);
  ...
  const savePenalty = async () => {
    if (!pForm.staffName || !pForm.amount) { alert('Staff + Amount ज़रूरी है'); return; }
    setSaving(true);
    try {
      await fetch(`${API}/api/staff-modules/penalties`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({...pForm, amount:+pForm.amount}) });
      setPForm(p=>({...p, amount:'', description:''}));
      load();
    } finally { setSaving(false); }
  };

  const saveIncentive = async () => {
    if (!iForm.staffName || !iForm.amount) { alert('Staff + Amount ज़रूरी है'); return; }
    setSaving(true);
    try {
      await fetch(`${API}/api/staff-modules/incentives`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({...iForm, amount:+iForm.amount, units:+iForm.units||1, perUnit:+iForm.perUnit||0}) });
      setIForm(i=>({...i, amount:'', description:'', units:'1', perUnit:''}));
      load();
    } finally { setSaving(false); }
  };
  ...
  const del = async (type, id) => {
    ...
    await fetch(`${API}/api/staff-modules/${type}/${id}`, { method:'DELETE' });
    load();
  };
  ...
  const leaderboard = useMemo(() => {
    ...
    incentives.forEach(i => {
      if (!map[i.staffName]) map[i.staffName] = { name:i.staffName, incentive:0, penalty:0, byType:{} };
      map[i.staffName].incentive += i.amount;
      map[i.staffName].byType[i.category] = (map[i.staffName].byType[i.category]||0)+i.amount;
    });
    penalties.forEach(p => {
      if (!map[p.staffName]) map[p.staffName] = { name:p.staffName, incentive:0, penalty:0, byType:{} };
      map[p.staffName].penalty += p.amount;
    });
    return Object.values(map).sort((a,b)=>(b.incentive-b.penalty)-(a.incentive-a.penalty)).map((s,i)=>({...s,rank:i+1,net:s.incentive-s.penalty}));
  }, [incentives, penalties]);

  const penaltyCats = ['late_coming','unauthorized_leave','customer_complaint','policy_violation','traffic_fine','company_damage','mobile_use','dress_code','manual','other'];
  const incentiveCats = ['retail','finance','insurance','accessories','service','exchange','target','festival','custom','other'];
  const catLabel = c => ({ late_coming:'⏰ देर से आना', unauthorized_leave:'🚫 बिना permission छुट्टी', customer_complaint:'😠 Customer Complaint', policy_violation:'📋 Policy Violation', traffic_fine:'🚦 Traffic Fine', company_damage:'🔧 Company Damage', mobile_use:'📱 Mobile Use', dress_code:'👔 Dress Code', manual:'✍️ Manual', other:'➕ Other', retail:'🏍️ Retail', finance:'🏦 Finance', insurance:'🛡️ Insurance', accessories:'🎿 Accessories', service:'🔧 Service', exchange:'🔄 Exchange', target:'🎯 Target', festival:'🎉 Festival', custom:'✨ Custom' }[c] || c);

  const C = { bg:'#020617', surface:'#0f172a', border:'#1e293b', text:'#fff', muted:'#94a3b8', danger:'#ef4444', success:'#22c55e', warning:'#f59e0b' };
  const inSt = { background:'#020617', border:'1px solid #334155', color:'#fff', borderRadius:8, padding:'8px 10px', fontSize:12, outline:'none', width:'100%', boxSizing:'border-box' };
  const lbSt = { fontSize:10, color:'#94a3b8', fontWeight:600, display:'block', marginBottom:3 };

  const MONTHS = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];

  return (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, padding:16, marginTop:16 }}>
      {/* Header */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, flexWrap:'wrap', gap:8 }}>
        <h2 style={{ margin:0, fontSize:18, fontWeight:900, color:'#fff' }}>⚡ Staff Modules</h2>
        <div style={{ display:'flex', gap:6 }}>
          <select value={month} onChange={e=>setMonth(+e.target.value)} style={{ ...inSt, width:'auto', padding:'6px 10px' }}>
            {MONTHS.map((m,i)=><option key={i} value={i+1}>{m}</option>)}
          </select>
          <select value={year} onChange={e=>setYear(+e.target.value)} style={{ ...inSt, width:'auto', padding:'6px 10px' }}>
            {[2024,2025,2026,2027].map(y=><option key={y} value={y}>{y}</option>)}
          </select>
          <button onClick={load} style={{ background:'#DC0000', border:'none', color:'#fff', padding:'6px 12px', borderRadius:8, cursor:'pointer', fontSize:12, fontWeight:700 }}>↻</button>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display:'flex', gap:6, marginBottom:14, flexWrap:'wrap' }}>
        {[['penalty','⚠️ Penalty'],['incentive','🏆 Incentive'],['leaderboard','🏅 Leaderboard']].map(([t,l])=>(
          <button key={t} onClick={()=>setTab(t)} style={{ padding:'7px 16px', borderRadius:8, border:`1px solid ${tab===t?'#DC0000':C.border}`, background:tab===t?'#7f1d1d22':C.bg, color:tab===t?'#fca5a5':'#94a3b8', cursor:'pointer', fontSize:12, fontWeight:700 }}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab==='penalty' && (
        <>
          {/* Add form */}
          <div style={{ background:C.bg, border:`1px solid #334155`, borderRadius:10, padding:14, marginBottom:14 }}>
            <p style={{ margin:'0 0 10px', fontWeight:700, color:'#fca5a5', fontSize:13 }}>➕ नई Penalty</p>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <div>
                <label style={lbSt}>👤 Staff</label>
                <select value={pForm.staffName} onChange={e=>setPForm(p=>({...p,staffName:e.target.value}))} style={inSt}>
              ...
              <div>
                ...
                <select value={pForm.category} onChange={e=>setPForm(p=>({...p,category:e.target.value}))} style={inSt}>
                  {penaltyCats.map(c=><option key={c} value={c}>{catLabel(c)}</option>)}
                </select>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <div>
                <label style={lbSt}>💰 Amount ₹</label>
                <input type="number" value={pForm.amount} onChange={e=>setPForm(p=>({...p,amount:e.target.value}))} style={inSt} placeholder="e.g. 100"/>
              </div>
              <div>
                <label style={lbSt}>📆 Date</label>
                <input type="date" value={pForm.date} onChange={e=>setPForm(p=>({...p,date:e.target.value}))} style={inSt}/>
              </div>
            </div>
            <div style={{ marginBottom:8 }}>
              <label style={lbSt}>📝 Description</label>
              <input type="text" value={pForm.description} onChange={e=>setPForm(p=>({...p,description:e.target.value}))} style={inSt} placeholder="Details..."/>
            </div>
            <button onClick={savePenalty} disabled={saving} style={{ background:saving?'#374151':'#DC0000', border:'none', color:'#fff', padding:'9px 20px', borderRadius:8, fontWeight:800, cursor:'pointer', fontSize:13 }}>
              {saving?'Saving...':'✅ Penalty Add करें'}
            </button>
          </div>

          {/* Penalty list */}
          {loading?<p style={{color:C.muted,textAlign:'center'}}>Loading...</p>:penalties.length===0?
            <p style={{color:C.muted,textAlign:'center',padding:20}}>इस महीने कोई penalty नहीं</p>:
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {penalties.map(p=>(
                <div key={p._id} style={{background:C.bg,border:`1px solid #334155`,borderRadius:8,padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13}}>{p.staffName} <span style={{fontSize:10,color:'#fca5a5',background:'#7f1d1d22',padding:'1px 8px',borderRadius:4}}>{catLabel(p.category)}</span></div>
                    <div style={{fontSize:11,color:C.muted}}>{p.description} · {new Date(p.date).toLocaleDateString('en-IN')}</div>
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    <span style={{fontWeight:800,color:'#fca5a5',fontSize:15}}>-{fmtINR(p.amount)}</span>
                    <button onClick={()=>del('penalties',p._id)} style={{background:'none',border:`1px solid #334155`,color:C.muted,padding:'4px 8px',borderRadius:6,cursor:'pointer',fontSize:11}}>🗑</button>
                  </div>
                </div>
              ))}
              <div style={{textAlign:'right',color:'#fca5a5',fontWeight:700,padding:'6px 4px'}}>
                Total: {fmtINR(penalties.reduce((s,p)=>s+p.amount,0))}
              </div>
            </div>
          }
        </>
      ...
      {tab==='incentive' && (
        <>
          <div style={{ background:C.bg, border:`1px solid #334155`, borderRadius:10, padding:14, marginBottom:14 }}>
            <p style={{ margin:'0 0 10px', fontWeight:700, color:'#86efac', fontSize:13 }}>➕ नया Incentive</p>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <div>
                <label style={lbSt}>👤 Staff</label>
                <select value={iForm.staffName} onChange={e=>setIForm(i=>({...i,staffName:e.target.value}))} style={inSt}>
                  <option value="">Select staff...</option>
                  {(staffList||[]).map(s=><option key={s.id||s.name} value={s.name}>{s.name}</option>)}
                </select>
              </div>
              <div>
                <label style={lbSt}>🎯 Category</label>
                <select value={iForm.category} onChange={e=>setIForm(i=>({...i,category:e.target.value}))} style={inSt}>
                  {incentiveCats.map(c=><option key={c} value={c}>{catLabel(c)}</option>)}
                </select>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:8 }}>
              <div>
                <label style={lbSt}>💰 Amount ₹</label>
                <input type="number" value={iForm.amount} onChange={e=>setIForm(i=>({...i,amount:e.target.value}))} style={inSt} placeholder="Total ₹"/>
              </div>
              <div>
                <label style={lbSt}>🔢 Units</label>
                <input type="number" value={iForm.units} onChange={e=>setIForm(i=>({...i,units:e.target.value}))} style={inSt} placeholder="1"/>
              </div>
              <div>
                <label style={lbSt}>₹ Per Unit (optional)</label>
                <input type="number" value={iForm.perUnit} onChange={e=>setIForm(i=>({...i,perUnit:e.target.value}))} style={inSt} placeholder="500"/>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
              <div><label style={lbSt}>📆 Date</label><input type="date" value={iForm.date} onChange={e=>setIForm(i=>({...i,date:e.target.value}))} style={inSt}/></div>
              <div><label style={lbSt}>📝 Note</label><input type="text" value={iForm.description} onChange={e=>setIForm(i=>({...i,description:e.target.value}))} style={inSt} placeholder="Details..."/></div>
            </div>
            {iForm.units>1&&iForm.perUnit>0&&<p style={{fontSize:11,color:'#fbbf24',margin:'0 0 8px'}}>Auto: {iForm.units} × ₹{iForm.perUnit} = ₹{(+iForm.units * +iForm.perUnit).toLocaleString('en-IN')}</p>}
            <button onClick={saveIncentive} disabled={saving} style={{ background:saving?'#374151':'#16a34a', border:'none', color:'#fff', padding:'9px 20px', borderRadius:8, fontWeight:800, cursor:'pointer', fontSize:13 }}>
              {saving?'Saving...':'✅ Incentive Add करें'}
            </button>
          </div>

          {loading?<p style={{color:C.muted,textAlign:'center'}}>Loading...</p>:incentives.length===0?
            <p style={{color:C.muted,textAlign:'center',padding:20}}>इस महीने कोई incentive नहीं</p>:
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {incentives.map(i=>(
                <div key={i._id} style={{background:C.bg,border:`1px solid #334155`,borderRadius:8,padding:'10px 14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:13}}>{i.staffName} <span style={{fontSize:10,color:'#86efac',background:'#14532d22',padding:'1px 8px',borderRadius:4}}>{catLabel(i.category)}</span></div>
                    <div style={{fontSize:11,color:C.muted}}>{i.description} · {new Date(i.date).toLocaleDateString('en-IN')}{i.units>1?` · ${i.units} units`:''}</div>
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    <span style={{fontWeight:800,color:'#86efac',fontSize:15}}>+{fmtINR(i.amount)}</span>
                    <button onClick={()=>del('incentives',i._id)} style={{background:'none',border:`1px solid #334155`,color:C.muted,padding:'4px 8px',borderRadius:6,cursor:'pointer',fontSize:11}}>🗑</button>
                  </div>
                </div>
              ))}
              <div style={{textAlign:'right',color:'#86efac',fontWeight:700,padding:'6px 4px'}}>
                Total: {fmtINR(incentives.reduce((s,i)=>s+i.amount,0))}
              </div>
            </div>
          }
        </>
      ...
      {tab==='leaderboard' && (
        <div>
          <p style={{color:C.muted,fontSize:12,marginBottom:12}}>🏅 {MONTHS[month-1]} {year} — Incentive rank (penalty deduct होगा)</p>
          {leaderboard.length===0?<p style={{color:C.muted,textAlign:'center',padding:30}}>इस महीने कोई data नहीं</p>:
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {leaderboard.map(s=>(
                <div key={s.name} style={{background:C.bg,border:`1px solid ${s.rank===1?'#fbbf24':s.rank===2?'#94a3b8':s.rank===3?'#f97316':C.border}`,borderRadius:10,padding:14,display:'flex',alignItems:'center',gap:12}}>
                  <div style={{width:36,height:36,borderRadius:'50%',background:s.rank===1?'#fbbf2433':s.rank===2?'#94a3b833':s.rank===3?'#f9731633':'#1e293b',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>
                    {s.rank===1?'🥇':s.rank===2?'🥈':s.rank===3?'🥉':`#${s.rank}`}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:800,fontSize:14,color:s.rank<=3?'#fff':'#e2e8f0'}}>{s.name}</div>
                    <div style={{fontSize:11,color:C.muted}}>
                      <span style={{color:'#86efac'}}>+{fmtINR(s.incentive)} incentive</span>
                      {s.penalty>0&&<span style={{color:'#fca5a5'}}> · -{fmtINR(s.penalty)} penalty</span>}
                    </div>
                    {Object.keys(s.byType).length>0&&(
                      <div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:4}}>
                        {Object.entries(s.byType).map(([cat,amt])=>(
                          <span key={cat} style={{fontSize:9,background:'#1e293b',color:'#94a3b8',padding:'1px 6px',borderRadius:4}}>{catLabel(cat)}: {fmtINR(amt)}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:18,fontWeight:900,color:s.net>=0?'#86efac':'#fca5a5'}}>{fmtINR(Math.abs(s.net))}</div>
                    <div style={{fontSize:10,color:C.muted}}>Net {s.net>=0?'earned':'deducted'}</div>
                  </div>
                </div>
              ))}
            </div>
