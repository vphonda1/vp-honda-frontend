...
function StaffModulesPanel({ staffList }) {
  const [tab,       setTab]       = React.useState('penalty');
  const [penalties, setPenalties] = React.useState([]);
  const [incentives,setIncentives]= React.useState([]);
  const [loading,   setLoading]   = React.useState(false);
  const [pForm,     setPForm]     = React.useState({ staffName:'', category:'late_coming', amount:'', description:'', date:new Date().toISOString().split('T')[0] });
  const [iForm,     setIForm]     = React.useState({ staffName:'', category:'retail', amount:'', units:'1', perUnit:'', description:'', date:new Date().toISOString().split('T')[0] });
