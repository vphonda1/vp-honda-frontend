...
const extractPDFText = async (file) => {
  ...
  // ✅ FIXED: Try /api/invoices/parse-pdf first, fallback to /api/parse-pdf
  let res = await fetch(api('/api/invoices/parse-pdf'), {
    method: 'POST',
    body: formData,
  });
  
  // Fallback to old endpoint if not found
  if (res.status === 404) {
    res = await fetch(api('/api/parse-pdf'), {
      method: 'POST',
      body: formData,
    });
  }
  ...
  // Handle both old format { text: '...' } and new format { invoice: {...} }
  let extractedText = data.text || data.rawText;
  
  if (!extractedText && !data.text) {
    // New format - backend already parsed
    if (data.invoice) {
  ...
  console.log('📄 PDF extracted via backend, length:', extractedText.length);
