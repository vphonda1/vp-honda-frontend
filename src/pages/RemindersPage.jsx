...
import {
  ...
  Shield, Wrench, CreditCard, Car, Search, RotateCcw, Plus, Filter, User, Calendar, Trash2,
} from 'lucide-react';
...
import { normKey, isRealRegNo, consolidateServiceData, displayRegNo } from '../utils/vehicleIdentity';
import { buildReminders as buildRemindersFromData, SERVICE_KEY_MAP, PRIORITY, reminderStats, NOTIFY_LADDER } from '../utils/reminderEngine';
...
export default function RemindersPage() {
  ...
  const [manual,      setManual]      = useState([]);   // हाथ से जोड़े गए reminders
  ...
  const [addForm,     setAddForm]     = useState({
    title:'', notes:'', dueDate:todayISO(), priority:'normal',
    customerName:'', phone:'', vehicle:'', regNo:'', assignedTo:'', type:'manual',
  });
  ...
  const buildReminders = useCallback(async () => {
    try {
      ...
      // ⭐ अब reminder बनाने का नियम एक ही जगह है — utils/reminderEngine.js.
      // पहले यह page, notificationScheduler और Vercel cron तीनों अपने-अपने
      // अलग नियमों से reminder बनाते थे, इसलिए तीनों की गिनती अलग आती थी.
      // हाथ से जोड़े गए reminders भी साथ लाओ
      let man = [];
      try { const r = await fetch(api('/api/reminders')); if (r.ok) { const d = await r.json(); if (Array.isArray(d)) man = d; } } catch {}
      setManual(man);

      // ⭐ Showroom leads की "अगली call" भी यहीं दिखे
      let leads = [];
      try { const r = await fetch(api('/api/visitors')); if (r.ok) { const d = await r.json(); if (Array.isArray(d)) leads = d; } } catch {}

      const { reminders: built, mergeInfo } = buildRemindersFromData(
        getLS('customerServiceData', {}),
        Array.isArray(custs) ? custs : [],
        getLS('followUpLog', {}),
        man,
        leads,
      );
      setReminders(built);
      if (mergeInfo?.length) setDupInfo(mergeInfo);
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      // staff list — reminder किसे सौंपना है, इसके लिए
  ...
  // एक बार तुरंत चलती है. यही सबसे बड़ी बचत है — service सो पाती है.
    intervalRef.current = visibleInterval(loadAll, 180000);   // 60s → 3 मिनट + hidden पर रुके
    return () => { window.removeEventListener('storage', loadAll); intervalRef.current?.(); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ── ⬅️ Android का Back बटन ────────────────────────────────────────────────
  // पहले modal खुला होने पर Back दबाने से पूरा app बंद हो जाता था.
  // अब Back पहले modal/filter बंद करता है, और तभी page छोड़ता है जब कुछ खुला न हो.
  const overlayOpen = showAdd || showDone || showFU || showFilters;
  useEffect(() => {
    // marker — इससे नया Service Worker बीच काम में reload नहीं करेगा
    if (overlayOpen) document.body.setAttribute('data-vp-modal-open', '1');
    else document.body.removeAttribute('data-vp-modal-open');
    if (!overlayOpen) return;
    // एक नकली history entry — Back इसी को खाएगा, page नहीं छोड़ेगा
    window.history.pushState({ vpOverlay: true }, '');
    const onPop = () => {
      setShowAdd(false); setShowDone(false); setShowFU(false); setShowFilters(false);
    };
    window.addEventListener('popstate', onPop);
    return () => {
      window.removeEventListener('popstate', onPop);
      // modal बटन से बंद हुआ — तो नकली entry खुद हटा दें
  ...
  // ── हाथ से जोड़े गए reminder MongoDB के `reminders` collection में हैं,
  //    इसलिए उनके complete/snooze का रास्ता अलग है ─────────────────────────
  const isManual = r => r?.source === 'manual';
  const isLead   = r => r?.source === 'lead';

  // ⚠️ Lead वाले reminder `visitors` collection में रहते हैं, servicedatas में
  // नहीं. इसलिए इनका "पूरा हुआ"/"बाद में" अलग रास्ते से जाना चाहिए — वरना
  // reminderState किसी गाड़ी के record में लिख जाता और कुछ असर ही न होता.
  const leadPatch = async (r, patch) => {
    try {
      const { updateOne } = await import('../utils/trackingStore');
      await updateOne('visitors', r.leadId, patch);
      return true;
    } catch { setSyncMsg('⚠️ नहीं हो पाया'); setTimeout(() => setSyncMsg(''), 3000); return false; }
  };

  const addManual = async () => {
    if (!addForm.title.trim()) { window.alert('Title लिखना ज़रूरी है'); return; }
    const st = staff.find(x => String(x._id || x.staffId) === addForm.assignedTo);
    try {
      const res = await fetch(api('/api/reminders'), {
        method:'POST', headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({
          ...addForm,
          assignedToName: st?.name || (addForm.assignedTo === (me.staffId || me.id) ? me.name : ''),
          createdBy: me.name || '',
        }),
      });
      if (!res.ok) throw new Error((await res.json()).error || 'सेव नहीं हुआ');
      setShowAdd(false);
      setAddForm({ title:'', notes:'', dueDate:todayISO(), priority:'normal',
        customerName:'', phone:'', vehicle:'', regNo:'', assignedTo:'', type:'manual' });
      setSyncMsg('✅ नया reminder जुड़ गया'); setTimeout(() => setSyncMsg(''), 3000);
      loadAll();
    } catch (e) { window.alert('⚠️ ' + e.message); }
  };

  const manualAction = async (r, path, body) => {
    try {
      const res = await fetch(api(`/api/reminders/${r.mongoId}/${path}`), {
        method:'PATCH', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify(body || {}),
      });
      if (!res.ok) throw new Error('नहीं हो पाया');
      return true;
    } catch { setSyncMsg('⚠️ नहीं हो पाया — internet देखें'); setTimeout(() => setSyncMsg(''), 3500); return false; }
  };

  const deleteManual = async (r) => {
    if (!window.confirm(`"${r.title}" को हमेशा के लिए मिटाएँ?`)) return;
    try { await fetch(api(`/api/reminders/${r.mongoId}`), { method:'DELETE' }); } catch {}
    setSyncMsg('🗑️ मिटा दिया'); setTimeout(() => setSyncMsg(''), 2500);
    loadAll();
  };
  ...
  const snooze = async (r, days) => {
    ...
    if (isLead(r)) {
      await leadPatch(r, { nextFollowUp: until.slice(0, 10) });
      setSyncMsg(`⏰ अगली call ${days} दिन बाद`); setTimeout(() => setSyncMsg(''), 3000);
      loadAll(); return;
    }
    if (isManual(r)) {
      const ok = await manualAction(r, 'snooze', { until: until.slice(0, 10) });
      if (ok) { setSyncMsg(`⏰ ${days} दिन के लिए टाल दिया`); setTimeout(() => setSyncMsg(''), 3000); }
      loadAll(); return;
    }
  ...
  const snoozeCustom = async (r) => {
    const d = window.prompt('📅 किस तारीख़ तक टालें? (YYYY-MM-DD)', todayISO());
    if (!d) return;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) { window.alert('सही format: YYYY-MM-DD'); return; }
    const days = Math.max(1, Math.ceil((new Date(d) - Date.now()) / DAY));
    snooze(r, days);
  };

  const closeReminder = async (r, reason) => {
    if (isLead(r)) {
      // call हो गई — अगली तारीख़ हटा दो, lead चालू ही रहेगा
      await leadPatch(r, { nextFollowUp: '' });
      setSyncMsg('✅ call हो गई — तारीख़ हटा दी'); setTimeout(() => setSyncMsg(''), 3000);
      loadAll(); return;
    }
    if (isManual(r)) {
      const ok = await manualAction(r, 'complete', { by: me.name || '' });
      if (ok) { setSyncMsg('✅ पूरा हुआ'); setTimeout(() => setSyncMsg(''), 3000); }
      loadAll(); return;
    }
  ...
  const reopenReminder = async (r) => {
    if (isManual(r)) {
      await manualAction(r, 'reopen', {});
      setSyncMsg('↩️ वापस चालू'); setTimeout(() => setSyncMsg(''), 2500);
      loadAll(); return;
    }
    const sd = getLS('customerServiceData', {});
  ...
  const submitDone = async () => {
    ...
    if (isLead(r)) {
      await leadPatch(r, {
        nextFollowUp: '',
        followUps: [...(r.leadFollowUps || []), {
          date:new Date().toISOString(), note: doneForm.remarks || 'call हुई',
          by: me.name || '', outcome:'contacted',
        }],
      });
      setShowDone(false); setActiveR(null);
      setSyncMsg('✅ call दर्ज हुई'); setTimeout(() => setSyncMsg(''), 3000);
      loadAll(); return;
    }
    if (isManual(r)) {
      await manualAction(r, 'complete', { by: me.name || '' });
      setShowDone(false); setActiveR(null);
      setSyncMsg('✅ पूरा हुआ'); setTimeout(() => setSyncMsg(''), 3000);
      loadAll(); return;
    }
    const sd = getLS('customerServiceData', {});
    if (!sd[r.regNo]) sd[r.regNo] = {};
  ...
  const FILTERS = [
    ...
    { t:'fu',   l:'📞 Follow-up',  n:cnt(r => r.type === 'followup'),             c:'#0ea5e9' },
    { t:'man',  l:'📌 अपने जोड़े',   n:cnt(r => r.source === 'manual'),             c:'#8b5cf6' },
    { t:'lead', l:'👤 Leads',       n:cnt(r => r.source === 'lead'),               c:'#14b8a6' },
    { t:'fix',  l:'⚠️ तारीख़ चाहिए', n:cnt(r => r.needsDate),                     c:'#d97706' },
  ];

  const stats = useMemo(() => reminderStats(reminders), [reminders]);

  const filtered = useMemo(() => (view === 'completed' ? parked : active).filter(r => {
    // तारीख़ की सीमा
    ...
    // किसे सौंपा गया
    if (userFilter === 'me'  && String(r.assignedTo || '') !== String(me.staffId || me.id || '')) return false;
    if (userFilter === 'none'&& r.assignedTo) return false;
    if (userFilter !== 'all' && userFilter !== 'me' && userFilter !== 'none'
        && String(r.assignedTo || '') !== userFilter) return false;
    // priority
    ...
    if (filterType === 'today'    && r.daysRemaining !== 0) return false;
    if (filterType === 'upcoming' && !(r.daysRemaining !== null && r.daysRemaining > 0)) return false;
    ...
    if (filterType === 'fu'   && r.type !== 'followup') return false;
    if (filterType === 'man'  && r.source !== 'manual') return false;
    if (filterType === 'lead' && r.source !== 'lead') return false;
    ...
    return true;
  }), [active, parked, view, filterType, searchTerm, dateFrom, dateTo, userFilter, prioFilter, me]);

  const filtersOn = dateFrom || dateTo || userFilter !== 'all' || prioFilter !== 'all';
  const clearFilters = () => { setDateFrom(''); setDateTo(''); setUserFilter('all'); setPrioFilter('all'); };
      ...
      <div id={`rem-${r.id}`} style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap' }}>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:7, flexWrap:'wrap', marginBottom:3 }}>
              ...
              {/* 🎯 Priority — due date के हिसाब से अपने आप तय होती है */}
              {PRIORITY[r.priority] && (
                <span style={{ background:`${PRIORITY[r.priority].color}1c`, border:`1px solid ${PRIORITY[r.priority].color}55`, color:PRIORITY[r.priority].color, fontSize:9, fontWeight:800, padding:'1px 7px', borderRadius:20 }}>
                  {PRIORITY[r.priority].label}
                </span>
              )}
              {r.source === 'manual' && <span style={{ background:'rgba(139,92,246,.12)', border:'1px solid rgba(139,92,246,.35)', color:'#c4b5fd', fontSize:9, fontWeight:700, padding:'1px 7px', borderRadius:20 }}>📌 अपना</span>}
              {r.assignedToName && <span style={{ background:'rgba(34,197,94,.1)', border:'1px solid rgba(34,197,94,.28)', color:'#86efac', fontSize:9, fontWeight:700, padding:'1px 7px', borderRadius:20 }}>👤 {r.assignedToName}</span>}
            </div>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:10 }}>
          ...
          <div style={{ display:'inline-flex', gap:3, alignItems:'center', background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.1)', borderRadius:10, padding:'3px 6px' }}>
            ...
            <button onClick={() => snooze(r, 1)}  style={{ background:'rgba(255,255,255,.06)', border:'none', borderRadius:6, color:'#cbd5e1', fontSize:10, fontWeight:700, padding:'2px 7px', cursor:'pointer' }}>कल</button>
            <button onClick={() => snooze(r, 3)}  style={{ background:'rgba(255,255,255,.06)', border:'none', borderRadius:6, color:'#cbd5e1', fontSize:10, fontWeight:700, padding:'2px 7px', cursor:'pointer' }}>3 दिन</button>
            <button onClick={() => snoozeCustom(r)} style={{ background:'rgba(255,255,255,.06)', border:'none', borderRadius:6, color:'#cbd5e1', fontSize:10, fontWeight:700, padding:'2px 7px', cursor:'pointer' }}>तारीख़…</button>
          </div>
          ...
          {isRealRegNo(r.regNo) && (
            <button onClick={() => navigate(`/veh-dashboard?search=${encodeURIComponent(normKey(r.regNo))}`)} style={S.btn('linear-gradient(135deg,#1e293b,#0f172a)')}>
              🏍 गाड़ी
            </button>
          )}
          {r.type === 'service' && (
            <button onClick={() => navigate(`/job-cards?search=${encodeURIComponent(r.customerName || '')}`)} style={S.btn('linear-gradient(135deg,#1e293b,#0f172a)')}>
              <Wrench size={10}/> सर्विस
            </button>
          )}
          {isManual(r) && (
            <button onClick={() => deleteManual(r)} style={S.btn('linear-gradient(135deg,#7f1d1d,#991b1b)')}>
              <Trash2 size={10}/> मिटाएँ
            </button>
          )}
          {isLead(r) && (
            <button onClick={() => navigate('/visitors')} style={S.btn('linear-gradient(135deg,#0d9488,#0f766e)')}>
              👤 Lead खोलें
            </button>
          )}
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding:'16px 18px 50px', maxWidth:1100, margin:'0 auto' }}>
        ...
        {/* 🔔 NOTIFICATION की सेहत — "कभी आते हैं कभी नहीं" खुद जाँचने के लिए */}
        {notifStatus === 'granted' && (
          <details style={{ background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.08)', borderRadius:12, padding:'10px 14px', marginBottom:12 }}>
            <summary style={{ color:'#94a3b8', fontSize:11.5, fontWeight:700, cursor:'pointer' }}>
              🔔 Notification कैसे और कब आएँगी — जाँचें
            </summary>
            <div style={{ marginTop:10, display:'grid', gap:8, fontSize:11, color:'#64748b' }}>
              <p style={{ margin:0 }}>
                <b style={{ color:'#cbd5e1' }}>सीढ़ी:</b> हर reminder पर{' '}
                {NOTIFY_LADDER.filter(d => d > 0).map(d => `${d} दिन पहले`).join(' → ')} → due के दिन → overdue.
                हर पड़ाव पर सिर्फ़ एक notification.
              </p>
              <p style={{ margin:0 }}>
                <b style={{ color:'#cbd5e1' }}>App खुली है:</b> तुरंत in-app ·{' '}
                <b style={{ color:'#cbd5e1' }}>App बंद है:</b> server push (Vercel Cron, दिन में 6 बार)
              </p>
              <div style={{ display:'flex', gap:7, flexWrap:'wrap', marginTop:3 }}>
                <button onClick={async () => {
                  try {
                    const r = await fetch(api('/api/push/health'));
                    const d = await r.json();
                    const per = Object.entries(d.devicesPerUser || {}).map(([k, v]) => `  ${k}: ${v}`).join('\n') || '  कोई नहीं';
                    window.alert(`🔔 Push की हालत\n\nRegistered devices: ${d.devicesSubscribed}\nDatabase: ${d.db}\n\nकिस user के कितने device:\n${per}\n\n${d.devicesSubscribed === 0 ? '⚠️ एक भी device registered नहीं — इसीलिए notification नहीं आ रही.' : '✅ ठीक है'}`);
                  } catch (e) { window.alert('⚠️ Backend से जवाब नहीं मिला: ' + e.message); }
                }} style={{ ...S.btn('rgba(255,255,255,.07)'), fontSize:11 }}>
                  📡 कितने device जुड़े हैं?
                </button>
                <button onClick={async () => {
                  try {
                    const reg = await navigator.serviceWorker.ready;
                    await reg.showNotification('✅ Test — VP Honda', {
                      body:'यह दिखी तो notifications ठीक काम कर रही हैं.',
                      icon:'/logo.png', badge:'/icons/icon-96x96.png', tag:'vp-test-' + Date.now(),
                      data:{ url:'/reminders' },
                    });
                  } catch (e) { window.alert('⚠️ ' + e.message); }
                }} style={{ ...S.btn('rgba(255,255,255,.07)'), fontSize:11 }}>
        ...
        {/* ═══ SMART DASHBOARD — Total | Today | Upcoming | Overdue | Completed ═══ */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(104px,1fr))', gap:9, marginBottom:12 }}>
          {[
            { k:'all',  l:'कुल',       v:stats.total,     c:'#3b82f6', go:() => { setView('pending'); setFilterType('all'); } },
            { k:'tdy',  l:'आज',        v:stats.today,     c:'#f59e0b', go:() => { setView('pending'); setFilterType('today'); } },
            { k:'up',   l:'आने वाले',   v:stats.upcoming,  c:'#22c55e', go:() => { setView('pending'); setFilterType('upcoming'); } },
            { k:'ovr',  l:'Overdue',   v:stats.overdue,   c:'#ef4444', go:() => { setView('pending'); setFilterType('over'); } },
            { k:'done', l:'पूरे हुए',   v:stats.completed, c:'#10b981', go:() => setView('completed') },
          ].map(x => {
            const on = (x.k === 'done' && view === 'completed') ||
                       (view === 'pending' && ((x.k === 'ovr' && filterType === 'over') ||
                        (x.k === 'tdy' && filterType === 'today') || (x.k === 'up' && filterType === 'upcoming') ||
                        (x.k === 'all' && filterType === 'all')));
            return (
              <button key={x.k} onClick={x.go} style={{
                background: on ? `${x.c}30` : `linear-gradient(135deg,${x.c}18,${x.c}06)`,
                border:`1px solid ${on ? x.c : x.c + '30'}`, borderRadius:14, padding:'11px 9px',
                textAlign:'center', cursor:'pointer', color:'#fff', font:'inherit',
              }}>
                <div style={{ fontSize:21, fontWeight:900, color:x.c, lineHeight:1 }}>{x.v}</div>
                <div style={{ fontSize:10, color:'#94a3b8', fontWeight:700, marginTop:3 }}>{x.l}</div>
              </button>
            );
          })}
        </div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12 }}>
          <button onClick={() => setShowAdd(true)} style={{ ...S.btn('linear-gradient(135deg,#DC0000,#991b1b)'), padding:'9px 14px', fontSize:12 }}>
            <Plus size={13}/> नया reminder
          </button>
          <button onClick={() => setShowFilters(v => !v)} style={{
            ...S.btn(filtersOn ? 'linear-gradient(135deg,#0284c7,#0369a1)' : 'rgba(255,255,255,.06)'),
            padding:'9px 14px', fontSize:12,
          }}>
            <Filter size={13}/> Filters{filtersOn ? ' •' : ''}
          </button>
          {view === 'completed' && (
            <button onClick={() => setView('pending')} style={{ ...S.btn('rgba(255,255,255,.06)'), padding:'9px 14px', fontSize:12 }}>
              ← बाक़ी reminders
            </button>
          )}
          {stats.critical > 0 && view === 'pending' && (
            <span style={{ background:'rgba(239,68,68,.14)', border:'1px solid rgba(239,68,68,.35)', color:'#fca5a5', borderRadius:10, padding:'9px 13px', fontSize:11.5, fontWeight:800 }}>
              🔴 {stats.critical} अति ज़रूरी
            </span>
          )}
        </div>
        ...
        {/* Filters panel */}
        {showFilters && (
          <div style={{ background:'rgba(255,255,255,.03)', border:`1px solid ${'rgba(255,255,255,.1)'}`, borderRadius:14, padding:14, marginBottom:12, display:'grid', gap:11, gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))' }}>
            <div>
              <label style={{ color:'#94a3b8', fontSize:10.5, fontWeight:700, display:'block', marginBottom:4 }}>📅 तारीख़ — से</label>
              <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} style={S.fld}/>
            </div>
            <div>
              <label style={{ color:'#94a3b8', fontSize:10.5, fontWeight:700, display:'block', marginBottom:4 }}>📅 तारीख़ — तक</label>
              <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} style={S.fld}/>
            </div>
            <div>
              <label style={{ color:'#94a3b8', fontSize:10.5, fontWeight:700, display:'block', marginBottom:4 }}>👤 किसे सौंपा</label>
              <select value={userFilter} onChange={e => setUserFilter(e.target.value)} style={S.fld}>
                <option value="all">सभी</option>
                <option value="me">सिर्फ़ मेरे</option>
                <option value="none">किसी को नहीं सौंपे</option>
                {staff.map(x => <option key={x._id} value={String(x._id || x.staffId)}>{x.name}</option>)}
              </select>
            </div>
            <div>
              <label style={{ color:'#94a3b8', fontSize:10.5, fontWeight:700, display:'block', marginBottom:4 }}>🎯 Priority</label>
              <select value={prioFilter} onChange={e => setPrioFilter(e.target.value)} style={S.fld}>
                <option value="all">सभी</option>
                {Object.entries(PRIORITY).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
            </div>
            {filtersOn && (
              <button onClick={clearFilters} style={{ ...S.btn('rgba(255,255,255,.06)'), alignSelf:'end', justifyContent:'center' }}>
                <X size={11}/> filters हटाएँ
              </button>
            )}
          </div>
        )}
      ...
      {/* ══ MODAL: नया reminder (हाथ से) ══ */}
      {showAdd && (
        <div style={S.modal} onClick={() => setShowAdd(false)}>
          <div style={S.mbox} onClick={e => e.stopPropagation()}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
              <h3 style={{ color:'#f1f5f9', fontWeight:800, fontSize:16, margin:0 }}>📌 नया Reminder</h3>
              <button onClick={() => setShowAdd(false)} style={{ background:'rgba(255,255,255,.06)', border:'none', borderRadius:'50%', width:30, height:30, color:'#94a3b8', cursor:'pointer' }}><X size={15}/></button>
            </div>
            <p style={{ color:'#64748b', fontSize:11.5, margin:'0 0 15px' }}>
              सर्विस, payment और insurance के reminder अपने आप बनते हैं. यह उन कामों के लिए है जो किसी data से नहीं बनते.
            </p>

            <div style={{ display:'grid', gap:11 }}>
              <div>
                <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>क्या करना है? *</label>
                <input value={addForm.title} onChange={e => setAddForm(f => ({ ...f, title:e.target.value }))}
                  placeholder="जैसे — श्री वर्मा को नई Shine दिखानी है" style={S.fld}/>
              </div>

              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>कब तक? *</label>
                  <input type="date" value={addForm.dueDate} onChange={e => setAddForm(f => ({ ...f, dueDate:e.target.value }))} style={S.fld}/>
                </div>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>Priority</label>
                  <select value={addForm.priority} onChange={e => setAddForm(f => ({ ...f, priority:e.target.value }))} style={S.fld}>
                    {Object.entries(PRIORITY).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div>
                <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>
                  👤 किसे करना है? <span style={{ color:'#475569', fontWeight:600 }}>— इनके सारे phone पर notification जाएगी</span>
                </label>
                <select value={addForm.assignedTo} onChange={e => setAddForm(f => ({ ...f, assignedTo:e.target.value }))} style={S.fld}>
                  <option value="">किसी को नहीं (सबको दिखेगा)</option>
                  <option value={me.staffId || me.id || ''}>मुझे — {me.name || 'मैं'}</option>
                  {staff.filter(x => String(x._id || x.staffId) !== String(me.staffId || me.id)).map(x => (
                    <option key={x._id} value={String(x._id || x.staffId)}>{x.name}{x.position ? ` (${x.position})` : ''}</option>
                  ))}
                </select>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>ग्राहक का नाम</label>
                  <input value={addForm.customerName} onChange={e => setAddForm(f => ({ ...f, customerName:e.target.value }))} placeholder="वैकल्पिक" style={S.fld}/>
                </div>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>Phone</label>
                  <input value={addForm.phone} onChange={e => setAddForm(f => ({ ...f, phone:e.target.value }))} placeholder="वैकल्पिक" style={S.fld}/>
                </div>
              </div>

              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>गाड़ी</label>
                  <input value={addForm.vehicle} onChange={e => setAddForm(f => ({ ...f, vehicle:e.target.value }))} placeholder="वैकल्पिक" style={S.fld}/>
                </div>
                <div>
                  <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>गाड़ी नंबर</label>
                  <input value={addForm.regNo} onChange={e => setAddForm(f => ({ ...f, regNo:e.target.value.toUpperCase() }))} placeholder="वैकल्पिक" style={S.fld}/>
                </div>
              </div>

              <div>
                <label style={{ color:'#94a3b8', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>टिप्पणी</label>
                <textarea value={addForm.notes} onChange={e => setAddForm(f => ({ ...f, notes:e.target.value }))} rows={2} placeholder="कोई ख़ास बात…" style={{ ...S.fld, resize:'vertical' }}/>
              </div>
            </div>

            <p style={{ color:'#7dd3fc', fontSize:10.5, background:'rgba(56,189,248,.07)', border:'1px solid rgba(56,189,248,.2)', borderRadius:10, padding:'8px 11px', margin:'14px 0 0' }}>
              🔔 Notification अपने आप जाएगी: {NOTIFY_LADDER.filter(d => d > 0).map(d => `${d} दिन पहले`).join(' → ')} → due के दिन → overdue
            </p>

            <div style={{ display:'flex', gap:9, marginTop:14 }}>
              <button onClick={() => setShowAdd(false)} style={{ flex:1, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.09)', borderRadius:13, padding:11, color:'#94a3b8', fontSize:12, fontWeight:700, cursor:'pointer' }}>रद्द करें</button>
              <button onClick={addManual} style={{ flex:1, background:'linear-gradient(135deg,#DC0000,#991b1b)', border:'none', borderRadius:13, padding:11, color:'#fff', fontSize:12, fontWeight:800, cursor:'pointer' }}>जोड़ें</button>
            </div>
          </div>
        </div>
      )}
