...
// Helper functions (same as original, keep as is)
...
export default function RemindersPage() {
  ...
  const loadAll = async () => {
    try {
      ...
      let dbInv=[];
      try{ const r=await fetch(api('/api/invoices')); if(r.ok) dbInv=await r.json(); }catch(e){}
      ...
      // 2. Sync service data from MongoDB
      try {
      ...
      // 3. Build from invoices
      buildServiceData(all);

      // 4. Sync back to MongoDB (fire and forget)
      try {
  ...
  const buildReminders = async () => {
    try {
      ...
      Object.entries(sd).forEach(([regNo, data]) => {
        ...
        // RTO (7 days after insurance date)
      ...
      // Update notification summary (for UI panel)
      const summary = {
        total: all.length,
      ...
      // ───── IMMEDIATE PUSH NOTIFICATIONS FOR OVERDUE/TODAY ─────
  ...
  // Follow-up and Service Done handlers (same as original)
  const submitFollowUp = async () => {
    ...
    setShowFU(false);
    setFuForm({status:'called',note:'',nextCallDate:''});
    setActiveR(null);
    buildReminders();
    setSyncMsg('✅ Follow-up saved');
  ...
  const submitDone = async () => {
    ...
    if(key) {
      sd[activeR.regNo][key+'Date'] = doneForm.date;
      sd[activeR.regNo][key+'Km'] = doneForm.km;
    }
    ...
    } catch(e){ setSyncMsg('⚠️ Local save only'); }
    setTimeout(()=>setSyncMsg(''),3000);
    setShowDone(false);
    setActiveR(null);
    setDoneForm({km:'', date:new Date().toISOString().split('T')[0], remarks:''});
    ...
    <div style={S.page}>
          ...
          <div style={{display:'flex', alignItems:'center', height:'34px'}}>
            ...
            <div style={{overflow:'hidden', flex:'1'}}>
              <div style={{display:'flex', gap:'28px', whiteSpace:'nowrap', animation: tickerPause ? 'none' : 'tk 40s linear infinite'}}>
                {todayCrit.map((r,i)=>(
                  <span key={i} style={{fontSize:'11px', color:'#fecaca', fontWeight:'600', display:'inline-flex', alignItems:'center', gap:'8px'}}>
                    🚨 {r.customerName} — {r.title} — {r.vehicle} ({r.regNo}) — {Math.abs(r.daysRemaining)}d overdue {r.customerPhone && <span style={{opacity:'.55'}}>📞{r.customerPhone}</span>}
                  </span>
                ))}
              </div>
            </div>
      ...
      <div style={S.hdr}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
          ...
          <button onClick={()=>{setLoading(true); setTimeout(loadAll,200);}} className="hov" style={S.btn('linear-gradient(135deg,#3b82f6,#1d4ed8)','0 4px 14px rgba(59,130,246,0.28)')}><RefreshCw size={13}/> Refresh</button>
        </div>
      </div>
      ...
      <div style={{padding:'18px 22px', maxWidth:'1200px', margin:'0 auto'}}>
        ...
        <div style={{ background: notifStatus==='granted' ? 'linear-gradient(135deg,#16a34a22,#16a34a08)' : 'linear-gradient(135deg,#1e3a8a22,#1e3a8a08)', border: `1px solid ${notifStatus==='granted' ? '#16a34a55' : '#3b82f655'}`, borderRadius:12, padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
          ...
          <div style={{flex:1, minWidth:200}}>
            {notifStatus==='granted' ? (
              <>
                ...
                {notifSummary ? (
                  <div style={{marginTop:6, display:'flex', gap:10, flexWrap:'wrap'}}>
                    ...
                    <div style={{background:'#fbbf2422', border:'1px solid #fbbf2455', borderRadius:6, padding:'3px 10px', display:'flex', gap:5, alignItems:'center'}}><span style={{color:'#fbbf24', fontWeight:800, fontSize:14}}>{notifSummary.overdue||0}</span><span style={{color:'#94a3b8', fontSize:10, fontWeight:600}}>🚨 Overdue</span></div>
                    <div style={{background:'#3b82f622', border:'1px solid #3b82f655', borderRadius:6, padding:'3px 10px', display:'flex', gap:5, alignItems:'center'}}><span style={{color:'#60a5fa', fontWeight:800, fontSize:14}}>{notifSummary.today||0}</span><span style={{color:'#94a3b8', fontSize:10, fontWeight:600}}>📅 आज</span></div>
                  </div>
              ...
              <><p style={{color:'#fca5a5', fontWeight:700, fontSize:13, margin:0}}>🔕 Notifications blocked हैं</p><p style={{color:'#64748b', fontSize:11, margin:'3px 0 0'}}>Browser settings में VP Honda को allow करें: Settings → Site Settings → Notifications</p></>
            ) : (
              <><p style={{color:'#93c5fd', fontWeight:700, fontSize:13, margin:0}}>📱 Phone पर Reminder Notifications चालू करें</p><p style={{color:'#64748b', fontSize:11, margin:'3px 0 0'}}>एक बार allow करें — service due होने पर automatic notification आएगी, app बंद हो तब भी</p></>
            )}
          </div>
          <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
            {notifStatus!=='granted' && notifStatus!=='denied' && (
              <button onClick={async()=>{ const granted = await requestNotificationPermission(); setNotifStatus(Notification.permission); if(granted) showInAppToast('🔔 Notifications enabled!','अब reminders automatic आएंगे','success'); }} style={{background:'#3b82f6', color:'#fff', border:'none', padding:'8px 16px', borderRadius:8, fontWeight:700, fontSize:12, cursor:'pointer', whiteSpace:'nowrap'}}>🔔 Allow Notifications</button>
            )}
            {notifStatus==='granted' && (
              <>
                <button onClick={async()=>{ try{ const res=await fetch('/api/test-push-notification',{method:'POST'}); const data=await res.json(); showInAppToast('📱 Test notification भेजी!', data.message||'Check your phone', 'success'); }catch(err){ showInAppToast('❌ Failed', err.message, 'error'); } }} style={{background:'#1e293b', color:'#94a3b8', border:'1px solid #334155', padding:'8px 12px', borderRadius:8, fontWeight:700, fontSize:11, cursor:'pointer', whiteSpace:'nowrap'}}>🧪 Test करें</button>
                <button onClick={async()=>{ /* optional re-schedule */ }} style={{background:'#16a34a', color:'#fff', border:'none', padding:'8px 12px', borderRadius:8, fontWeight:700, fontSize:11, cursor:'pointer', whiteSpace:'nowrap'}}>↻ Re-schedule</button>
              </>
            )}
          </div>
        </div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:'10px', marginBottom:'14px'}}>
          {[
            ...
            <button key={i} onClick={()=>navigate(btn.path)} className="hov" style={{background:btn.grad, border:'none', borderRadius:'12px', padding:'11px 14px', color:'#fff', fontSize:'12px', fontWeight:'700', cursor:'pointer', boxShadow:`0 3px 14px ${btn.shadow}`, textAlign:'center', letterSpacing:'0.2px'}}>{btn.label}</button>
          ))}
        </div>
        ...
        {filtered.length===0 ? (
          <div style={{textAlign:'center', padding:'72px 20px', animation:'fi 0.4s ease'}}>
            <div style={{width:'68px', height:'68px', borderRadius:'50%', background:'rgba(34,197,94,0.08)', border:'2px solid rgba(34,197,94,0.18)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px'}}><CheckCircle size={34} color="#22c55e"/></div>
                  ...
                  <div style={{padding:'14px 16px'}}>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'10px', flexWrap:'wrap', gap:'7px'}}>
                      <div style={{display:'flex', alignItems:'center', gap:'7px', flexWrap:'wrap'}}>
                        <span style={{background:isCrit?'rgba(239,68,68,0.14)':'rgba(234,179,8,0.1)', border:`1px solid ${isCrit?'rgba(239,68,68,0.35)':'rgba(234,179,8,0.28)'}`, color:isCrit?'#fca5a5':'#fde047', fontSize:'9px', fontWeight:'800', padding:'2px 8px', borderRadius:'20px', letterSpacing:'0.5px'}}>{isCrit?'● OVERDUE':'◐ UPCOMING'}</span>
                    ...
                    <div style={{display:'flex', gap:'14px', alignItems:'flex-start'}}>
                      <div style={{flex:1, minWidth:'0'}}>
                        ...
                        <div style={{display:'flex', flexWrap:'wrap', gap:'10px', alignItems:'center', padding:'9px 13px', background:'rgba(255,255,255,0.03)', borderRadius:'11px', border:'1px solid rgba(255,255,255,0.05)'}}>
                          <div style={{display:'flex', alignItems:'center', gap:'7px'}}>
                            ...
                            <div><p style={{color:'#e2e8f0', fontWeight:'700', fontSize:'12px', margin:'0'}}>{r.customerName}</p>{r.regNo && <p style={{color:'#475569', fontSize:'9px', margin:'0', fontFamily:'monospace'}}>{r.regNo}</p>}</div>
                          </div>
                        ...
                        {fups.length>0 && (
                          <button onClick={()=>setExpandedId(isExp?null:r.id)} style={{marginTop:'7px', background:'none', border:'1px solid rgba(34,197,94,0.18)', borderRadius:'8px', padding:'4px 11px', color:'#4ade80', fontSize:'10px', fontWeight:'600', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:'5px'}}><TrendingUp size={10}/> {fups.length} call logs {isExp?'▲':'▼'}</button>
                        )}
                      </div>
                      <div style={{display:'flex', flexDirection:'column', gap:'6px', flexShrink:'0'}}>
                        ...
                        {r.type==='insurance-renewal' && <button onClick={()=>{ const newDate=prompt(`📅 Insurance Date enter करें (YYYY-MM-DD):\n\nCurrent: ${r.insuranceStartDate||'Not set'}\n\nखरीद के 2-3 दिन बाद की date डालें`); if(newDate && /^\d{4}-\d{2}-\d{2}$/.test(newDate)){ const key=`vp_ins_${r.regNo||r.customerId}`; localStorage.setItem(key,newDate); alert(`✅ Insurance date saved!\n\nPage refresh करें — नया reminder calculate होगा।`); loadAll(); } }} className="hov" style={S.btn('linear-gradient(135deg,#0369a1,#0284c7)','0 2px 10px rgba(3,105,161,0.25)')}>✏️ Edit Date</button>}
                        {r.type==='insurance-renewal' && <button onClick={()=>{ if(window.confirm(`${r.customerName} का Insurance Renewed mark करना है?`)){ const key=`vp_ins_renewed_${r.regNo||r.customerId}`; localStorage.setItem(key,'true'); alert('✅ Marked as Renewed! अगले साल का reminder automatic set होगा।'); loadAll(); } }} className="hov" style={S.btn('linear-gradient(135deg,#16a34a,#15803d)','0 2px 10px rgba(22,163,74,0.25)')}>🛡️ Renewed</button>}
                      ...
                      <div style={{marginTop:'12px', background:'rgba(0,0,0,0.28)', borderRadius:'13px', padding:'13px', border:'1px solid rgba(255,255,255,0.05)', animation:'fi 0.2s ease'}}>
                        ...
                        {fups.map((f,i)=>(
                          <div key={i} style={{display:'flex', gap:'11px', padding:'7px 0', borderBottom:i<fups.length-1?'1px solid rgba(255,255,255,0.04)':'none', alignItems:'flex-start'}}>
                            <div style={{flexShrink:'0', minWidth:'62px'}}><div style={{color:'#64748b', fontSize:'9px'}}>{fmtDate(f.date)}</div><div style={{color:'#1e293b', fontSize:'8px'}}>{fmtTime(f.date)}</div></div>
                            <div style={{flex:1}}><span style={{fontWeight:'700', fontSize:'10px', color:csColor(f.status)}}>{csLabel(f.status)}</span>{f.note && f.note!=='—' && <div style={{color:'#94a3b8', fontSize:'10px', marginTop:'2px'}}>💬 {f.note}</div>}{f.nextCallDate && <div style={{color:'#fdba74', fontSize:'9px', marginTop:'2px'}}>📅 अगली: {fmtDate(f.nextCallDate)}</div>}</div>
        ...
        <div style={{marginTop:'18px', background:'rgba(255,255,255,0.02)', borderRadius:'18px', border:'1px solid rgba(255,255,255,0.05)', overflow:'hidden'}}>
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%', borderCollapse:'collapse', fontSize:'11px'}}>
              <thead><tr style={{borderBottom:'1px solid rgba(255,255,255,0.05)'}}>{['#','Invoice','Customer','Vehicle','Amount','Date',''].map(h=>(<th key={h} style={{textAlign:'left', color:'#334155', padding:'9px 13px', fontWeight:'700', fontSize:'10px', textTransform:'uppercase', letterSpacing:'0.5px'}}>{h}</th>))}</tr></thead>
              <tbody>
                {invoices.slice(0,6).map((inv,i)=>(
                  <tr key={i} style={{borderBottom:'1px solid rgba(255,255,255,0.03)', cursor:'pointer', transition:'background 0.12s'}} onClick={()=>navigate(`/invoice/${inv.invoiceNumber||inv.id}`)} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.025)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
        ...
        <div style={S.modal} onClick={()=>setShowFU(false)}>
          <div style={S.mbox} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'7px', marginBottom:'14px'}}>
              {CALL_STATUS.map(s=>(
                <button key={s.value} onClick={()=>setFuForm(f=>({...f,status:s.value}))} style={{background:fuForm.status===s.value ? ({called:'linear-gradient(135deg,#16a34a,#15803d)', promised:'linear-gradient(135deg,#2563eb,#1d4ed8)', no_answer:'linear-gradient(135deg,#ca8a04,#a16207)', busy:'linear-gradient(135deg,#ea580c,#c2410c)', later:'linear-gradient(135deg,#7c3aed,#6d28d9)', not_int:'linear-gradient(135deg,#dc2626,#b91c1c)'}[s.value]) : 'rgba(255,255,255,0.04)', border:fuForm.status===s.value ? 'none' : '1px solid rgba(255,255,255,0.07)', borderRadius:'10px', padding:'9px', color:'#fff', fontSize:'11px', fontWeight:'700', cursor:'pointer', transition:'all 0.18s'}}>{s.label}</button>
              ))}
            </div>
            <p style={{color:'#334155', fontSize:'10px', fontWeight:'700', marginBottom:'7px', textTransform:'uppercase', letterSpacing:'0.5px'}}>💬 Remarks</p>
            <textarea value={fuForm.note} onChange={e=>setFuForm(f=>({...f,note:e.target.value}))} placeholder="बात किसके साथ हुई, क्या बोले, कब आएंगे..." style={{width:'100%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:'11px', padding:'11px', color:'#f1f5f9', fontSize:'12px', height:'72px', resize:'none', outline:'none', boxSizing:'border-box', fontFamily:'inherit', marginBottom:'13px'}}/>
            ...
            <input type="date" value={fuForm.nextCallDate} onChange={e=>setFuForm(f=>({...f,nextCallDate:e.target.value}))} style={{width:'100%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:'11px', padding:'9px 13px', color:'#f1f5f9', fontSize:'12px', outline:'none', boxSizing:'border-box', marginBottom:'18px'}}/>
            <div style={{display:'flex', gap:'9px'}}><button onClick={()=>setShowFU(false)} style={{flex:1, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:'13px', padding:'11px', color:'#94a3b8', fontSize:'12px', fontWeight:'700', cursor:'pointer'}}>Cancel</button><button onClick={submitFollowUp} style={{flex:1, background:'linear-gradient(135deg,#7c3aed,#6d28d9)', border:'none', borderRadius:'13px', padding:'11px', color:'#fff', fontSize:'12px', fontWeight:'800', cursor:'pointer', boxShadow:'0 4px 18px rgba(124,58,237,0.35)'}}>✅ Save Log</button></div>
          </div>
        </div>
      )}
