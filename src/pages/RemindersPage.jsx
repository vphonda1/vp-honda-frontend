...
export default function RemindersPage() {
  ...
  // ✅ Notification click से आए URL params (?focus=Name&phone=...)
  ...
  // ✅ Notification click से आए URL params - उसे search box में डाल दे
  useEffect(() => {
    if (focusName) {
      setSearchTerm(focusName);
      setFilterType('all');
      setCurrentPage(1);
      // 1 second wait so reminders load + DOM render
      const t = setTimeout(() => {
        const el = document.getElementById(`reminder-${focusPhone || focusName}`);
        if (el) {
          el.scrollIntoView({ behavior: 'smooth', block: 'center' });
          el.style.boxShadow = '0 0 0 4px #fbbf24, 0 0 30px #fbbf24';
          el.style.transform = 'scale(1.02)';
          el.style.transition = 'all 0.3s';
          setTimeout(() => { el.style.boxShadow = ''; el.style.transform = ''; }, 4000);
        }
        showInAppToast('🔔 Notification', `${focusName} का reminder खोला`, 'success');
      }, 1500);
      return () => clearTimeout(t);
    }
  }, [focusName, focusPhone, reminders.length]);
  ...
  const buildReminders = async () => {
    try {
      ...
      // ⭐ FIX: Page के अपने all[] से summary बनाएं (scheduler के buildAllReminders में field mismatch है)
      if (all.length > 0) {
        const todayStr = today.toISOString().slice(0, 10);
        const tmrwDate = new Date(today); tmrwDate.setDate(tmrwDate.getDate() + 1);
        ...
        const summary = {
          ...
          today:    all.filter(r => dStr(r.dueDate) === todayStr).length,
          tomorrow: all.filter(r => dStr(r.dueDate) === tmrwStr).length,
          upcoming: all.filter(r => r.daysRemaining > 0).length,
          byType: {
            service:          all.filter(r => r.type === 'service').length,
            payment:          all.filter(r => r.type === 'payment').length,
            rto:              all.filter(r => r.type === 'insurance').length,           // RTO uses 'insurance' type in page logic
          ...
          <div style={{display: 'flex', gap: 8, flexWrap: 'wrap'}}>
            ...
            {notifStatus === 'granted' && (
              <>
                ...
                <ReminderPushButton reminders={reminders}/>
                <button onClick={async () => {
        ...
        {filtered.length===0?(
          ...
          <div>
            ...
            {paginated.map((r,idx)=>{
              ...
              return(
                <div key={r.id} id={`reminder-${(r.customerPhone||'').replace(/[^0-9]/g,'').slice(0,10) || r.customerName}`} className="rc" style={S.card(isCrit)}>
