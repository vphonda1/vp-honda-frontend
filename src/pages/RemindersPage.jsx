...
export default function RemindersPage() {
      ...
      try {
        ...
        if (sdRes.ok) {
          ...
          (Array.isArray(dbSD) ? dbSD : []).forEach(rec => {
            const reg = rec.regNo; if (!reg) return;
  ...
  const groups = useMemo(() => {
    ...
    arr.forEach(g => {
      ...
      g.vehicles = [...new Set(g.items.map(i => i.regNo).filter(Boolean))];
    });
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap' }}>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:7, flexWrap:'wrap', marginBottom:3 }}>
              ...
              {r.regNo && <span style={{ color:'#475569', fontSize:10, fontFamily:'monospace' }}>{r.regNo}</span>}
                    ...
                    <div style={{ padding:'0 15px 12px', display:'flex', gap:7, flexWrap:'wrap' }}>
                      {g.items.map(r => (
                        <span key={r.id} style={{ background:`${(TYPE_META[r.type]||{}).color}1c`, border:`1px solid ${(TYPE_META[r.type]||{}).color}44`, color:'#e2e8f0', fontSize:10.5, fontWeight:700, padding:'3px 9px', borderRadius:20 }}>
                          {r.title} · {r.daysRemaining === null ? 'तारीख़ नहीं' : r.daysRemaining < 0 ? `${Math.abs(r.daysRemaining)}d ऊपर` : `${r.daysRemaining}d`}
                        </span>
                      ))}
                    </div>
                  )}
