...
import { loadAll, saveOne, deleteOne } from '../utils/trackingStore';
...
// ⚠️ पहले appointments सिर्फ़ localStorage में थीं — cache साफ़ होते ही सब ख़त्म,
// और दूसरे phone पर दिखती ही नहीं थीं. अब server पर भी जाती हैं.
const loadA = () => { try { return JSON.parse(localStorage.getItem('vp_appointments') || '[]'); } catch { return []; } };
const saveA = (a) => { try { localStorage.setItem('vp_appointments', JSON.stringify(a)); } catch {} };
...
export default function CalendarView() {
  ...
  // server से ताज़ा list — न मिले तो phone वाली copy चलती रहेगी
  useEffect(() => {
    loadAll('appointments')
      .then(rows => { if (Array.isArray(rows) && rows.length) { setAppts(rows); saveA(rows); } })
      .catch(() => {});
  }, []);
  ...
  const addAppt = () => {
    ...
    // server पर भी — offline queue से होकर, इसलिए internet न हो तो भी नहीं खोती
    saveOne('appointments', a).catch(() => {});
  ...
  const delAppt = (id) => {
    ...
    const u = appts.filter(a => a.id !== id);
    saveA(u); setAppts(u);
    deleteOne('appointments', id).catch(() => {});
  };
  const remind = (a) => {
