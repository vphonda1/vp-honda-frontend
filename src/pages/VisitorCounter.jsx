...
import { loadAll, updateOne } from '../utils/trackingStore';
...
export default function VisitorCounter() {
  ...
  const refresh = async () => {
    ...
    // ⚠️ पहले सिर्फ़ localStorage से पढ़ता था. अब server से — इसलिए showroom के
    // हर phone/laptop पर वही list दिखती है.
    const all = await loadAll('visitors');
    setVisitors(all);
  };
  ...
  const handleConvert = async (visitor) => {
    if (window.confirm(`${visitor.name} को customer के रूप में convert करना है?`)) {
      // server पर भी दर्ज हो, वरना दूसरे phone पर फिर से "नया" दिखेगा
      await updateOne('visitors', visitor.localId || visitor.id, {
        converted: true, convertedAt: new Date().toISOString(), stage: 'won',
      });
