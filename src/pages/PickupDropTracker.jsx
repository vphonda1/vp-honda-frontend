...
import { loadAll } from '../utils/trackingStore';
import {
...
export default function PickupDropTracker() {
  ...
  // एक बार तुरंत चलती है. यही सबसे बड़ी बचत है — service सो पाती है.
    const stopPoll = visibleInterval(refresh, 180000);   // 30s → 3 मिनट
    return stopPoll;
  }, []);

  const refresh = () => {
    setStats(getPickupDropStats());
    // ⚠️ पहले सिर्फ़ localStorage से. अब server से — हर phone पर वही list.
    loadAll('pickupDrops').then(setList).catch(() => {
