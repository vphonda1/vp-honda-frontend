...
function buildNotificationSchedule(reminders) {
  ...
  // ── Priority sort: zyada overdue + payment type = upar ──
  const typePriority = { payment: 3, service: 2, insurance: 1, 'insurance-renewal': 1 };
  const overdueList = reminders
    .filter(r => r.status === 'overdue')
    .sort((a, b) => {
      const aPri = (Math.abs(a.daysRemaining || 0) * 10) + (typePriority[a.type] || 0);
      const bPri = (Math.abs(b.daysRemaining || 0) * 10) + (typePriority[b.type] || 0);
      return bPri - aPri;
    });

  const todayList = reminders.filter(r => r.dueDate === todayStr && r.status !== 'overdue');
  const tmrwList  = reminders.filter(r => r.dueDate === tmrwStr  && r.status !== 'overdue');

  // ── 1. SUMMARY notification (एक single — counts एक साथ) ──
  if (overdueList.length || todayList.length || tmrwList.length) {
    const parts = [];
    if (overdueList.length) parts.push(`🚨 ${overdueList.length} overdue`);
    if (todayList.length)   parts.push(`📅 ${todayList.length} today`);
    if (tmrwList.length)    parts.push(`⏰ ${tmrwList.length} tomorrow`);
    scheduled.push({
      id:    `notif-summary-${todayStr}`,
      title: '🔔 VP Honda — Reminders Summary',
      body:  parts.join(' · ') + '\n📋 Tap to open',
      type:  'summary',
      fireAt: new Date(now.getTime() + 3000).toISOString(),
      tag:   'summary',
  ...
  // ── 2. Top 5 OVERDUE (most urgent first) ──
  const MAX_OVERDUE = 5;
  overdueList.slice(0, MAX_OVERDUE).forEach((r, idx) => {
    ...
    scheduled.push({
      ...
      type:  r.type,
      fireAt: new Date(now.getTime() + 6000 + idx * 3000).toISOString(), // 6s + 3s gap
      tag:   `ov-${r.id}`,
  ...
  // ── 3. Top 3 DUE TODAY ──
  const MAX_TODAY = 3;
  todayList.slice(0, MAX_TODAY).forEach(r => {
    const cfg = NOTIF_CONFIG[r.type] || NOTIF_CONFIG['service'];
    scheduled.push({
      id:    `notif-td-${r.id}`,
  ...
  // ── 4. Top 2 TOMORROW (kal 10 AM fire) ──
  const MAX_TMRW = 2;
  tmrwList.slice(0, MAX_TMRW).forEach(r => {
    const cfg = NOTIF_CONFIG[r.type] || NOTIF_CONFIG['service'];
    scheduled.push({
      id:    `notif-tm-${r.id}`,
...
export async function scheduleReminderNotifications(customers) {
  try {
    ...
    // ⚠️ FIX: पहले यहाँ `/api/customers` से data लिया जाता था — पर reminder की सारी
    // dates (purchaseDate, paymentDueDate, insuranceDate, 4th–7th service) असल में
    // `servicedatas` collection में रहती हैं, Customer schema में हैं ही नहीं.
    // इसलिए यह function लगभग हमेशा 0 reminders बनाता था और कोई local notification
    // नहीं आती थी. अब RemindersPage वाला ही source इस्तेमाल होता है.
    if (!Array.isArray(customers) || customers.length === 0) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(sd)) customers = sd;
        }
      } catch (e) { console.warn('[Notif] service-data fetch failed:', e.message); }

      // Fallback: service-data खाली हो तो customers से ही कोशिश करें
      if (customers.length === 0) {
        try {
          const res = await fetch(`${API}/api/customers`);
          if (res.ok) {
            const cu = await res.json();
            if (Array.isArray(cu)) customers = cu;
          }
        } catch (e) { console.warn('[Notif] customers fetch failed:', e.message); }
      }

      console.log(`[Notif] Self-fetched ${customers.length} records`);
      if (customers.length === 0) return { reminders: [], scheduled: [] };
    }
