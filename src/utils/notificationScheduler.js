...
export async function scheduleReminderNotifications(customers) {
  try {
    ...
    // ✅ FIX: अगर customers data नहीं आया, खुद fetch करें
    if (!Array.isArray(customers) || customers.length === 0) {
      try {
        ...
        console.log(`[Notif] Self-fetched ${customers?.length || 0} customers`);
      } catch (e) {
        console.warn('[Notif] Customer fetch failed:', e.message);
