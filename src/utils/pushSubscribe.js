...
export async function ensurePushSubscription(forcePrompt = false) {
  try {
    ...
    // ⭐ किस user का device है — इसी से assigned reminder सही आदमी के सारे
    //    phone/laptop पर पहुँचता है
    let u = {};
    try { u = JSON.parse(localStorage.getItem('vpSession') || localStorage.getItem('vpHondaUser') || '{}') || {}; } catch {}
    const payload  = {
      ...sub.toJSON(), deviceId,
      userId:   u.staffId || u.id || '',
      userName: u.name || '',
