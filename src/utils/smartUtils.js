...
export const recordVisitor = async (visitorData) => {
  ...
  // ⚠️ FIX: पहले यह सिर्फ़ localStorage में जाता था — cache साफ़ होते ही पूरा
  // enquiry register ख़त्म. अब पहले phone में (UI तुरंत दिखे), फिर server पर
  // offline queue से होकर — internet न हो तो रुककर बाद में चला जाता है.
  visitor.localId = visitor.id;
  const existing = JSON.parse(localStorage.getItem('vp_visitors') || '[]');
  existing.unshift(visitor);
  localStorage.setItem('vp_visitors', JSON.stringify(existing.slice(0, 1000)));

  try {
    const { saveOne } = await import('./trackingStore');
    await saveOne('visitors', visitor);
  } catch (e) { console.warn('visitor server-sync:', e.message); }
...
export const recordPickupDrop = async (data) => {
  ...
  // ⚠️ FIX: पहले सिर्फ़ localStorage में जाता था. अब server पर भी —
  // offline queue से होकर, इसलिए internet न हो तो भी नहीं खोता.
  entry.localId = entry.id;
  entry.scheduled = entry.pickupTime;
  const existing = JSON.parse(localStorage.getItem('vp_pickup_drops') || '[]');
  existing.unshift(entry);
  localStorage.setItem('vp_pickup_drops', JSON.stringify(existing.slice(0, 500)));

  try {
    const { saveOne } = await import('./trackingStore');
    // photos (base64) server पर मत भेजो — MongoDB free tier भर जाएगा
    const { photos, ...light } = entry;
    await saveOne('pickupDrops', { ...light, photoCount: (photos || []).length });
  } catch (e) { console.warn('pickup-drop server-sync:', e.message); }
...
export const updatePickupDrop = (id, updates) => {
  ...
  // ⚠️ FIX: status बदलना (in-transit → completed) पहले सिर्फ़ इसी phone पर
  // दिखता था. अब server पर भी जाता है — driver का phone बदले तो भी हाल पता रहे.
  (async () => {
    try {
      const { updateOne } = await import('./trackingStore');
      const { photos, ...light } = updates;
      await updateOne('pickupDrops', id, light);
    } catch {}
  })();
