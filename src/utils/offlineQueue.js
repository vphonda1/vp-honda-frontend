...
export function initOfflineQueue() {
  ...
  // ⏱️ Render का कोटा: पहले हर 2 मिनट बिना शर्त चलता था और कभी clear नहीं
  // होता था. अब — tab सामने हो, internet हो, **और कतार में कुछ हो** तभी
  // चलता है. कतार खाली रहती है (आम हालत), इसलिए एक भी request नहीं जाती.
  const timer = setInterval(async () => {
    if (document.hidden || !navigator.onLine) return;
    if ((await pendingCount()) > 0) flushQueue();
  }, 5 * 60 * 1000);
  ...
  if (navigator.onLine) setTimeout(() => pendingCount().then(n => n > 0 && flushQueue()), 3000);
  notify();
}
