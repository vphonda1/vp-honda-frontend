...
// ⚠️ FIX: पहले यह component `reminders` prop लेता ही नहीं था और हमेशा
// `/api/reminders` से fetch करता था — वह एक अलग (लगभग खाली) Mongo collection है,
// RemindersPage वाले computed reminders नहीं. नतीजा: हमेशा "Generic push" जाता था.
// अब parent से आया हुआ reminders prop पहले इस्तेमाल होता है.
export default function ReminderPushButton({
  reminders: propReminders,
  style = {},
  label = '🔔 Send Top 5 Reminders to All',
}) {
  ...
  const sendPush = async () => {
    ...
    try {
      // Step 1: पहले parent (RemindersPage) से आए reminders इस्तेमाल करें
      let reminders = Array.isArray(propReminders) ? propReminders : [];
      if (reminders.length === 0) {
        try {
          const r = await fetch(api('/api/reminders'));
          if (r.ok) {
            const d = await r.json();
            if (Array.isArray(d)) reminders = d;
          }
      ...
      for (let i = 0; i < top.length; i++) {
        ...
        // 250ms gap (पहले 800ms था — 6 push में 4 सेकंड लग जाते थे)
