...
export default function Navbar({ user, onLogout }) {
  ...
  // ⚠️ अब कुछ links में ?tab= भी है (Dashboard के tabs), इसलिए pathname + search दोनों देखें
  const isActive = (p) => (location.pathname + location.search) === p || location.pathname === p;
  ...
  const mainItems = [
    ...
    { label:'Profit/Loss',   path:'/dashboard?tab=profit',show: isAdmin, icon:'💹' },
  ...
  const smartItems = [
    { label:'Payroll & Staff', path:'/dashboard?tab=payroll', show: isAdmin, icon:'👔' },
    { label:'Insights',        path:'/dashboard?tab=insights',show: isAdmin, icon:'🧠' },
    ...
    { label:'Receipts',        path:'/received-payments',    show: isStaff, icon:'🧾' },
...
/**
 * 📶 Offline / sync की हालत — हमेशा दिखने वाला छोटा बिल्ला
 *
 * ⚠️ Offline queue चुपचाप काम कर रही थी. User form भरता, "सेव हो गया" मानकर
 * आगे बढ़ जाता — जबकि record असल में कतार में रुका होता और उसे पता ही नहीं
 * चलता कि internet जोड़ना है. अब साफ़ दिखता है कि कितना काम बाक़ी है.
 */
function SyncStatus() {
  ...
  useEffect(() => {
    ...
    const readCount = async () => {
      try {
        const { pendingCount } = await import('../utils/offlineQueue');
    ...
    const onQueue = (e) => setPending(e.detail?.pending ?? 0);
    const goOn  = () => { setOnline(true);  readCount(); };
    const goOff = () => setOnline(false);
    const onSwMsg = (e) => { if (e.data?.type === 'QUEUE_FLUSHED') readCount(); };

    window.addEventListener('vp-queue-change', onQueue);
    window.addEventListener('online', goOn);
    window.addEventListener('offline', goOff);
    navigator.serviceWorker?.addEventListener?.('message', onSwMsg);
    const t = setInterval(readCount, 20000);

    return () => {
      alive = false;
      window.removeEventListener('vp-queue-change', onQueue);
  ...
  const sendNow = async () => {
    setBusy(true);
    try {
      const { flushQueue } = await import('../utils/offlineQueue');
  ...
  // सब ठीक है और कुछ रुका नहीं — कुछ मत दिखाओ
  if (online && pending === 0) return null;

  const offlineMode = !online;
  return (
    <button
      onClick={pending > 0 && online ? sendNow : undefined}
      title={offlineMode ? 'Internet नहीं है — काम सुरक्षित रखा जा रहा है'
                         : `${pending} काम भेजना बाक़ी — दबाकर अभी भेजें`}
      style={{
        display:'flex', alignItems:'center', gap:5, flexShrink:0,
        background: offlineMode ? 'rgba(239,68,68,.22)' : 'rgba(245,158,11,.22)',
        border: `1px solid ${offlineMode ? 'rgba(252,165,165,.5)' : 'rgba(253,224,71,.5)'}`,
        color:'#fff', borderRadius:999, padding:'3px 9px',
        fontSize:10.5, fontWeight:800, whiteSpace:'nowrap',
        cursor: pending > 0 && online ? 'pointer' : 'default',
      }}
    >
      {offlineMode
        ? <>📴 Offline{pending > 0 ? ` · ${pending} रुके` : ''}</>
        : busy ? <>⏳ भेज रहे हैं…</> : <>⏳ {pending} भेजना बाक़ी</>}
    </button>
  );
}
      ...
      <div className="max-w-full mx-auto px-3">
        <div className="flex justify-between items-center h-12">
          ...
          <Link to="/dashboard" className="flex items-center gap-2 flex-shrink-0">
            ...
            />
            <span className="text-white font-black text-sm hidden sm:block">VP Honda</span>
          </Link>
          ...
          {/* Mobile — offline/sync बिल्ला + hamburger */}
          <div className="lg:hidden flex items-center gap-1.5">
            <SyncStatus />
          <button onClick={() => setOpen(!open)} className="p-2 rounded hover:bg-white/20 text-white">
